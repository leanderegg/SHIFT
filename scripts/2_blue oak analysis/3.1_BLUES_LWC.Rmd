...
MuMIn::r.squaredGLMM(space_mod_site)

var(space_df$water_potential, na.rm=T) #0.940, 
vest_lin <- data.frame(VarCorr(space_mod_site))
sum(vest_lin$vcov) + 0.09 #1.001

space_linear_vest_df <- vest_lin %>% 
  add_row(vcov = r2m, 
          grp = "marginal_r2") %>% 
...
m1 <- lmer(mean_mpa ~ lma_g_cm2_new + time + (1|tree) + (lma_g_cm2_new|tree), data = lma_mpa_df, REML = F)
summary(m1)
MuMIn::r.squaredGLMM(m1)
anova(m1)
AIC(m1)

m2 <- lm(mean_mpa ~ time + (1|tree) + (lma_g_cm2_new|tree), data = lma_mpa_df, REML = F)
summary(m2)
MuMIn::r.squaredGLMM(m2)
...
#just pds: 
m2 <- lmer(mean_mpa ~ lma_g_cm2_new + week + (lma_g_cm2_new|tree), data = lma_mpa_df_pd, REML = F)
...
m3 <- lm(mean_mpa ~ week, data = lma_mpa_df_pd, REML = F)
m3 <- lmer(mean_mpa ~ week + (lma_g_cm2_new|tree), data = lma_mpa_df_pd, REML = F)
summary(m3)
...
m5 <- lmer(mean_mpa ~ lma_g_cm2_new + (lma_g_cm2_new|tree), data = lma_mpa_df_pd, REML = F)

#Store the R-squared of the 'better' model
marginal_r2_pd <- round(MuMIn::r.squaredGLMM(m5)[1], digits = 3)
marginal_r2_pd
```
...
lma_mpa_df_md <- lma_mpa_df %>% 
  filter(time == "md")
#just mds: 
m2 <- lmer(mean_mpa ~ lma_g_cm2_new + week + (lma_g_cm2_new|tree), data = lma_mpa_df_md, REML = F)
...
m3 <- lmer(mean_mpa ~ week + (lma_g_cm2_new|tree), data = lma_mpa_df_md, REML = F)
summary(m3)
MuMIn::r.squaredGLMM(m3)
anova(m3)
AIC(m3)
...
#just mds: 
m4 <- lmer(mean_mpa ~ lma_g_cm2_new + (lma_g_cm2_new|tree), data = lma_mpa_df_md, REML = F)
#Store the R-squared of the 'better' model
