...
# Iterate through each sampled tree and perform bootstrapping separately
      for (tree_id in sampled_tree_ids) {
       ...
       mutate(
        ...
        # Merge the CI values back into the original dataframe
      tbootstrapped_slopes_df_dcwc <- ci_values 
      
# Print the updated dataframe with CI values
print(tbootstrapped_slopes_df_dcwc)

summary(tbootstrapped_slopes_df_dcwc)

mean_slope_dcwc = tbootstrapped_slopes_df_dcwc %>% 
  mutate(mean_slope = mean(mean, na.rm = T),
         pre_post = "dcwc leafout") %>% 
  select(mean_slope, lower_bound, upper_bound, pre_post) %>% 
  distinct()
mean_slope_dcwc

dcwc_space <- tbootstrapped_slopes_df_dcwc %>% 
  select(tree, slope) %>% 
  distinct()
...
df <- dcwc_df %>% 
  filter(time == "md") %>% 
  mutate(date_wp = as.factor(date_wp))

m1 <- lmer(water_potential ~ slope*date_wp + (1|tree), data = df)
summary(m1)
...
m2 <- lmer(water_potential ~ slope*date_wp + (1|tree), data = df_pd)
summary(m2)
```
Just June/July (29-33)
...
# Number of random trees to sample
n_sample_trees <- 5  # Adjust as needed based on the number of unique trees
