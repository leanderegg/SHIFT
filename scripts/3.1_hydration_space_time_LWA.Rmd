...
wk10_wide <- wk10 %>% 
  pivot_wider(names_from=time, 
    ...
    dplyr::group_by(tree) %>%
  fill(c(lwa_g_cm2_md, lwa_g_cm2_pd, water_potential_pd, water_potential_md, date_wp), .direction = "downup") %>%
  dplyr::ungroup()%>% 
  select(-date_wp) %>% 
  distinct()

wk10_long_lwa_g_cm2 <- wk10_wide %>% 
...
wk10_long <- merge(wk10_long_mpa, wk10_long_lwa_g_cm2)
```
...
wk_figs <- wk_all_long %>% 
  filter(week %in% c(10, 14, 17, 19, 21, 29, 33), 
         lwa_g_cm2 < 6, 
         lwa_g_cm2 > 0
         ) %>% 
...
space_mod_site <- lmer(water_potential ~ lwa_g_cm2_centered  + (lwa_g_cm2_centered|week) + (1|tree) + (1|site), data = wc_wk_centered_df,  REML = T)
