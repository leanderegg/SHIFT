...
```{r, warning = F}
wk_all_wide_lwa_df <-  wc_qudo_lwa_df %>% 
  drop_na(date_wp) %>% 
...
wk_all_long_lwa_g_cm2 <- wk_all_wide_lwa_df %>% 
...
wk_all_long_mpa <- wk_all_wide_lwa_df %>% 
  drop_na(lwa_g_cm2_pd, lwa_g_cm2_md, water_potential_md, water_potential_pd) %>% 
...
```{r, include = F}
wk_all_sites <-  wc_qudo_lwa_df %>% mutate(site = as.factor(site)) %>% 
  drop_na(site)
...
```{r, echo=FALSE}
ran_list2 <- coef(m_wk_rand_nosite)
...
ran_err <- arm::se.ranef(m_wk_rand_nosite)
...
```{r}
ranef(m_wk_rand_nosite, condVar=T)

sjPlot::plot_model(m_wk_rand, type = "re", show.p = T)

sjPlot::plot_model(m_wk_rand, #type = "re", 
                   show.p = T)
```
...
```{r, warning = F}
wc_wk_centered_pd_df <- wc_qudo_lwa_df %>% 
  filter(time == "pd") %>% 
  group_by(week) %>% 
