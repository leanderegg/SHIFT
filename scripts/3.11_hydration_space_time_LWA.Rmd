...
```
#Bootsrapped dataset: 
Run: here::here("scripts", "3.10_bootstrapping_LWA.Rmd")
```{r}
final_df_all_lwa <- read_csv(here::here("processed-data", "bootstrapped_df_lwa.csv"))
```
#HYDRATION
...
ggsave(here("figures", "nice_hydration_lwa_ints"), hyd_ints, device = "jpg", width = 5, height = 2.5, dpi = 300)
```
...
ggsave(here("figures", "nice_hydration_slopes_bs_lwa_inds"), hyd_slopes_bs_inds, device = "jpg", width = 5, height = 2.5, dpi = 300)
```
...
time_season_mod <- lmer(water_potential ~ lwa_g_cm2 + time_season + (1|week) + (lwa_g_cm2|tree), data = wc_qudo_md_df)

time_season_mod_int <- lmer(water_potential ~ lwa_g_cm2*time_season + (1|week) + (lwa_g_cm2|tree), data = wc_qudo_md_df)

time_mod <- lmer(water_potential ~ lwa_g_cm2 + (1|week) + (lwa_g_cm2|tree), data = wc_qudo_md_df)
time_mod
AIC(time_mod)
```
...
time_mod <- lmer(water_potential ~ lwa_g_cm2 + (1|week) + (lwa_g_cm2|tree), data = wc_qudo_md_df)
#time_mod <- lmer(water_potential ~ lwa_g_cm2 + (lwa_g_cm2|tree), data = wc_qudo_md_df)
...
lin_time_mod <- lmer(water_potential ~ lwa_g_cm2 + (1|week) + (lwa_g_cm2|tree), data = lin_wc_qudo_md_df)
...
bef_time_mod <- lmer(water_potential ~ lwa_g_cm2 + (1|week) + (lwa_g_cm2|tree), data = bef_wc_qudo_md_df, REML = F)
...
```{r}
time_summary_bs_tree <- final_df_all_lwa %>% 
   filter(!week %in% c(9)) %>% 
...
```{r}
lwa_mods <- bind_rows(space_mod_df <- space_mod_df %>% mutate(mod = "space"),
...
write_csv(lwa_mods, here::here("processed-data", "lwa_mods.csv"))
```
