...
plotly::ggplotly(wc_long_lwa_all2 %>%  #masses look correct
  ...
  facet_wrap(~week, scales = "free") +
    geom_abline() +
    ggtitle("Wk 19 is different because we reweighed LWCs and replaced them after calculating the 'lwc old' metric")
  ) 
```
...
wc_lwa_df_wk10 <- wc_lwa_df %>% 
  filter(week == 10) %>% 
  distinct()

unique(wc_lwa_df_wk10$tree)
...
investigating_wk10 <- wp_df  %>% 
  filter(week == 10 & time %in% c("md")) %>% 
  distinct()
...
#when there are bigish differences, replace the combined bulk+leaf with just the bulk; otherwise take the mean of both. 
  ...
  select(-date_wc, -lwc_leaf_mean, -lwc_bulk_mean, -lwc_leaf, -lwc_bulk, -dm_leaf_g, -wm_leaf_g, -dry_scan_area, -lwc_bulk_leaf_diff) %>% 
...
investigating_wk10 <-wp_wc_rwc_alas_df1  %>% 
  filter(week == 10 & time %in% c("md")) %>% 
  distinct()
...
investigating_wk10 <-wp_wc_rwc_alas_df  %>% 
  filter(week == 10 & time %in% c("md")) %>% 
  distinct()
