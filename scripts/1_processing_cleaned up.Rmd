...
#MiddleEarth
  ...
  filter(!(tree %in% c(2365) & date_wp %in% c("2022-03-23") & time %in% c("pd") & mpa > 2)) %>% #notes say PDs were weird because taken eve of next day
  filter(!(tree %in% c(2377) & date_wp %in% c("2022-03-23") & time %in% c("md") & mpa >= 2.55)) %>% #notes say these were "old leaves ugly"
  filter(!(tree %in% c() & date_wp %in% c() & time %in% c() & mpa == )) %>% 
  filter(!(tree %in% c() & date_wp %in% c() & time %in% c() & mpa == )) %>% 
  filter(!(tree %in% c() & date_wp %in% c() & time %in% c() & mpa == )) %>% 
