...
wc_df <- read_csv(here("processed-data", paste0("wc_alldates_",datver,".csv")), show_col_types = FALSE) %>% #contains LWC data
  ...
  distinct() %>% 
  mutate(wm_leaf_g = case_when(
   wm_leaf_g %in% c(1.6180) ~ (0.1618),
   TRUE ~ as.numeric(wm_leaf_g)
  ),
 lwc_leaf = case_when(
   tree %in% c(2345) & week %in% c(10) & rep %in% c(2) & time %in% c("md")~ (0.96867),
...
lwa_df <- wc_lwa_df %>% 
 # select(-rep) %>% 
  ...
  mutate(lwa_g_cm2_new = case_when(
        week %in% c(13) & tree %in% c(2009, 2370) & time %in% c("pd") ~ NA_real_, 
  ...
  mutate(lwc_new = case_when(
    ...
    tree %in% c(2343) & week %in% c(15) & time %in% c('md') ~ NA_real_,
     tree %in% c(2370) & week %in% c(29) & time %in% c("md") ~ NA_real_,
      tree %in% c(2331) & week %in% c(15) & time %in% c("pd") ~ NA_real_,
    tree %in% c(2091) & week %in% c(19) & time %in% c("md") ~ NA_real_,
    tree %in% c(2354) & week %in% c(15) & time %in% c("md") ~ NA_real_,
    tree %in% c(2345) & week %in% c(10) & time %in% c("md") ~ NA_real_, 
...
#Issue with weights, 
  ...
  mutate(date = date_wc) %>% 
#when we didn't measure exactly on the same day, but want predawns and middays to overlap, change the dates (need to do this for MPas too)
  mutate(date = case_when( 
    date %in% c("2022-05-25") ~ as.Date("2022-05-23"), 
    date %in% c("2022-04-27") ~ as.Date("2022-04-25"), 
  ...
  mutate(tree = as.numeric(tree), 
     ...
     tree == 2374 ~ 2376, 
     tree == 2364 ~ 2360, 
     tree == 2004 & date %in% c("2022-07-19") ~ 2005,
     tree == 2014 & date %in% c("2022-09-15") ~ 2013,
     species %in% c("ARCA") & site %in% c("Cucu Mesa") ~ 12,
     TRUE ~ as.numeric(tree))) %>% 
  mutate(week = case_when(
    date %in% c("2022-03-30") ~ 14, #this was being listed as week 13, but I think it should actually be 14 based on 4/4 and 4/6 samplings having some midday trees that correspond to the predawns measured on this day. 
    TRUE ~ week(date))) 
...
#when we didn't measure exactly on the same day, but want predawns and middays to overlap, change the dates (need to do this for LWC too)
  mutate(date_wp = case_when(
   ...
   date_wp %in% c("2022-04-27") ~ as.Date("2022-04-25"), 
    TRUE ~ as.Date(date_wp))) %>% 
  distinct()

#Look at it to see what measurements are oddballs: 

#LL and MiddleEarth
plotly::ggplotly(wp_df %>% 
                   filter(plot == "MiddleEarth") %>% 
                   ggplot(aes(y = mpa, 
                     x = as.factor(tree), 
                     color = time)) +
  geom_point() + 
  facet_wrap(~as.factor(date_wp)))

#Hobbiton
plotly::ggplotly(wp_df %>% 
                   filter(plot == "Hobbiton") %>% 
                   ggplot(aes(y = mpa, 
                     x = as.factor(tree), 
                     color = time)) +
  geom_point() + 
...
#Cucu, low
plotly::ggplotly(wp_df %>% 
                   filter(plot == "Low" & site == "Cucu Mesa") %>% 
                   ggplot(aes(y = mpa, 
                     x = as.factor(tree), 
                     color = time)) +
  geom_point() + 
...
plotly::ggplotly(wp_df %>% 
                   ...
                   group_by(tree, time, date_wp) %>% 
                   mutate(mpa = mean(mpa, na.rm = T)) %>% 
  mutate(tree_num = as.numeric(tree)) %>% 
  ggplot(aes(y =mpa, 
             x = as.character(tree), 
             shape = species,
             color = time)) +
  geom_point(alpha = .3) + 
  facet_wrap(~date_wp*week, nrow = 5))
```

Remove outliers from individual mpa measurements: 
...
#MiddleEarth
  filter(!(tree %in% c(2354) & date_wp %in% c("2022-02-28") & time %in% c("md") & mpa %in% c(2.63, 0.962))) %>% 
  filter(!(tree %in% c(2377) & date_wp %in% c("2022-02-28") & time %in% c("md") & mpa %in% c(2.71))) %>% 
  filter(!(tree %in% c(2352, 3454) & date_wp %in% c("2022-03-08") & time %in% c("md") & mpa %in% c(3.5, 2.9))) %>% 
  filter(!(tree %in% c(3454) & date_wp %in% c("2022-03-08") & time %in% c("pd") & mpa > 2)) %>% 
  filter(!(tree %in% c(2366) & date_wp %in% c("2022-03-08") & time %in% c("md") & mpa == 3)) %>% 
  filter(!(tree %in% c(2376) & date_wp %in% c("2022-03-08") & time %in% c("md") & mpa %in% c(4.306)))%>% 
  filter(!(tree %in% c(2377) & date_wp %in% c("2022-03-08") & time %in% c("pd"))) %>% #notes say these were hard to read and they're insanely high
  filter(!(tree %in% c(2380) & date_wp %in% c("2022-03-08") & time %in% c("pd") & mpa < .6)) %>%
  filter(!(tree %in% c(2365) & date_wp %in% c("2022-03-15") & time %in% c("pd") & mpa < .5)) %>% 
  filter(!(tree %in% c(2365) & date_wp %in% c("2022-03-25") & time %in% c("pd") & mpa > 2)) %>% #notes say PDs were weird because taken eve of next day
  filter(!(tree %in% c(2377) & date_wp %in% c("2022-03-25") & time %in% c("md") & mpa >= 2.55)) %>% #notes say these were "old leaves ugly"
  filter(!(tree %in% c(2377) & date_wp %in% c("2022-04-11") & time %in% c("pd") & mpa == 0.3 )) %>% 
  filter(!(tree %in% c(2354) & date_wp %in% c("2022-04-13") & time %in% c("md") & mpa > 3.5)) %>% #super negative relative to all other measurements
  #filter(!(tree %in% c(2379) & date_wp %in% c("2022-04-13") & time %in% c("md") & mpa > 3)) %>% #mpas are split around 2 and around 3; not notes which to trust. Take average? 
  filter(!(tree %in% c(2352) & date_wp %in% c("2022-05-09") & time %in% c("pd") & mpa > 3 )) %>% #1/5 pds is >3, rest are < 2
  filter(!(tree %in% c(2376) & date_wp %in% c("2022-08-18") & time %in% c("pd") & mpa > 3)) %>% 
  filter(!(tree %in% c(2354) & date_wp %in% c("2022-09-15") & time %in% c("pd") & mpa == 2.9)) %>% 
  
#Hobbiton:
   filter(!(tree %in% c(2347) & date_wp %in% c("2022-02-28") & time %in% c("pd") & mpa > 2 )) %>% 
  filter(!(tree %in% c(2347) & date_wp %in% c("2022-03-08") & time %in% c("md") & mpa > 3 )) %>% 
   filter(!(tree %in% c(2346) & date_wp %in% c("2022-03-15") & time %in% c("pd") & mpa > 3 )) %>% 
   filter(!(tree %in% c(2347) & date_wp %in% c("2022-03-15") & time %in% c("md") & mpa > 3 )) %>% 
  mutate(time = case_when(
    tree %in% c(2345) & date_wp %in% c("2022-03-25") & mpa %in% c(1.800, 2.700, 3.100, 3.450) ~ "md", 
    tree %in% c(2345) & date_wp %in% c("2022-03-25") & mpa %in% c(1.850, 2.050, 1.300, 1.500, 2.500, 2.400, 2.4500) ~ "pd", #predawn and middays entered in wrong spot? 
    TRUE ~ as.character(time))
  ) %>% 
  filter(!(tree %in% c(2301) & date_wp %in% c("2022-04-04") & time %in% c("pd") & mpa > 3)) %>%
  filter(!(tree %in% c(2345) & date_wp %in% c("2022-08-18") & time %in% c("pd") & mpa > 3)) %>% #note says these were y0 and they're also WAY more negative than the middays. 

#Weathertop
  filter(!(tree %in% c(2381) & date_wp %in% c("2022-03-02") & time %in% c("md") & mpa > 2)) %>%
  filter(!(tree %in% c(2381) & date_wp %in% c("2022-03-02") & time %in% c("md") & mpa < .5)) %>%
  filter(!(tree %in% c(2381) & date_wp %in% c("2022-03-15") & time %in% c("md") & mpa < .5)) %>%
    filter(!(tree %in% c(2371) & date_wp %in% c("2022-03-30") & time %in% c("pd") & mpa < .5)) %>%
  filter(!(tree %in% c(2384) & date_wp %in% c("2022-04-06") & time %in% c("md") & mpa > 2.5)) %>% #notes say "injured leaf" and "? broke"
  filter(!(tree %in% c(2382) & date_wp %in% c("2022-04-11") & time %in% c("pd") & mpa > 2 )) %>% #way more neg than MDs, and we did a ton of reps likely due to such weird values
  filter(!(tree %in% c(2010) & date_wp %in% c("2022-07-09") & time %in% c("md") & mpa == 3.00)) %>%
  filter(!(tree %in% c(2383) & date_wp %in% c("2022-09-15") & time %in% c("pd") & mpa < 0.4 )) %>%

#ridge
  filter(!(tree %in% c(2331) & date_wp %in% c("2022-07-19") & time %in% c("pd") & mpa > 3)) %>%
    
#Cucu, high
  filter(!(tree %in% c(2006) & date_wp %in% c("2022-09-15") & time %in% c("pd") & mpa > 3)) %>%
  
#Cucu, low
  filter(!(tree %in% c(2015) & date_wp %in% c("2022-07-09") & time %in% c("pd") & mpa > 4 )) %>%
  filter(!(tree %in% c(2012) & date_wp %in% c("2022-05-23") & time %in% c("md") & mpa < 2.7 )) %>%
  
#Piney (In general a LOT of variation in PDs and MDs at Piney, probable because of the Y1 vs. Y0 differences, which are not well noted except in the notebooks)
  filter(!(tree %in% c(2086) & date_wp %in% c("2022-04-04") & time %in% c("md"))) %>% #not sure what this tree is, we didnt seen to collect middays at Piney on this day; just predawns. Are we missing a sheet with the Piney middays? 
  filter(!(tree %in% c(2089) & date_wp %in% c("2022-04-27") & time %in% c("md") & mpa < 2 )) %>%
  filter(!(tree %in% c(2092) & date_wp %in% c("2022-05-23") & time %in% c("md") & mpa < 2 )) %>%
  filter(!(tree %in% c(2092) & date_wp %in% c("2022-09-15") & time %in% c("pd") & mpa > 1.5)) %>%
  
#Chamise: 
  filter(!(tree %in% c(2026) & date_wp %in% c("2022-04-04") & time %in% c("md") & mpa > 3.4 )) %>%
  filter(!(tree %in% c(2029) & date_wp %in% c("2022-05-09") & time %in% c("pd") & mpa > 2)) %>%
  mutate(date_wp = ymd(date_wp)) %>% 
  mutate(week = case_when(
    date_wp %in% c("2022-03-30") ~ 14, #this was being listed as week 13, but I think it should actually be 14 based on 4/4 and 4/6 samplings having some midday trees that correspond to the predawns measured on this day. 
    TRUE ~ week(date_wp))) %>% 
  arrange(date_wp, tree, time)
```

Check that we didn't lose more MPas than planned: 
```{r}
plotly::ggplotly(wp_df_2 %>% 
                   filter(species == "live oak"
                          #week %in% c(12, 14, 15)
                          ) %>% 
                   group_by(tree, time, date_wp) %>% 
  ...
  ggplot(aes(y =mpa, 
             ...
             color = time)) +
  geom_point(alpha = .3) + 
  ggtitle("After removing MPa outliers") +
  facet_wrap(~date_wp*week, nrow = 5))
```
...
plotly::ggplotly(wp_df_2 %>% 
                  # filter(site == "Chamise") %>% 
                   ggplot(aes(y = mpa, 
                     ...
                     color = time)) +
  geom_point() + 
  facet_wrap(~as.factor(date_wp)*week))


#Pivot wider to view: 
wp_df2_wider <- wp_df_2 %>% 
  select(site, week, time, mpa, tree, date_wp) %>% 
                   pivot_wider(names_from = "time", 
                               values_from = "mpa", 
                               values_fn = function(x) mean(x, na.rm = TRUE)) %>% 
  filter(tree > 100)

# plotly::ggplotly(wp_df2_wider %>%  
#                    ggplot(aes(y = md, 
#                      x = pd, 
#                      color = site, 
#                      label = tree
...
#all PDs are less negative than MDs, thats good!!

#write out to send to Lee: 
write_csv(wp_df_2, here::here("processed-data", "wp_cleanedup_20240516.csv"))
#write out to send to Lee: 
write_csv(wp_df2_wider, here::here("processed-data", "wp_wide_cleanedup_20240516.csv"))
```
...
plotly::ggplotly(wp_mean_df %>% 
  ...
  ggplot(aes(y =mean_mpa, 
             ...
             color = time)) +
  geom_point(alpha = .3) + 
  ggtitle("Original, Just MPAs") +
  facet_wrap(~date_wp*week, nrow = 5))
```


```{r}
wp_wc_df2 <- merge(wp_mean_df, 
                   lwa_df %>% 
                     mutate(date = case_when(
                       date %in% c("2022-09-12") ~ as.Date("2022-09-15"),
                       TRUE ~ as.Date(date)
                     )), 
  ...
  distinct() %>% 
#deal with LWA outliers:
  mutate(lwa_g_cm2_new = case_when(
    week %in% c(19) & tree %in% c(2384) & time %in% c("pd") ~ NA,
    week %in% c(19) & tree %in% c(2089) ~ NA,
     week %in% c(14) & tree %in% c(2013, 2014, 2008, 2370) & time %in% c("md") ~ NA,
    TRUE ~ as.numeric(lwa_g_cm2_new)
  )) %>% 
#LWA outliers are likely the same as the LWC outliers: 
     mutate(lwc_bulk = case_when(
    week %in% c(19) & tree %in% c(2384) & time %in% c("pd") ~ NA,
    week %in% c(19) & tree %in% c(2089) ~ NA,
     week %in% c(14) & tree %in% c(2013, 2014, 2008, 2370) & time %in% c("md") ~ NA,
...
#  filter(!(week %in% c(37) & tree %in% c(2086) & time %in% c("pd"))) %>% 
 # filter(!(week %in% c(14) & tree %in% c(2013, 2014, 2008, 2370) & time %in% c("md"))) %>% #LWC is weird for these, might have been mislabled? 2370 just had one mpa rep
...
plotly::ggplotly(wp_wc_df2 %>% 
  ...
  ggplot(aes(y =mean_mpa, 
             ...
             color = time)) +
  geom_point(alpha = .3) + 
    ggtitle("After adding LWC and LWA") +
  facet_wrap(~date_wp*week, nrow = 5))
```
...
wp_wc_df3 <- bind_rows(wp_wc_wk37, wp_wc_no_wk37) %>% 
  mutate(date_wp = case_when(
    week %in% c(14) & time %in% c("md") & date_wp %in% c("<NA>", NA) ~ as.Date("2022-04-06"),
    week %in% c(37) & time %in% c("md") & date_wp %in% c("<NA>", NA) ~ as.Date("2022-09-15"),
    TRUE ~ as.Date(date_wp)
   ))
...
plotly::ggplotly(wp_wc_df3  %>% 
                   filter(week == 17 & tree %in% c(2009:2030)) %>% 
  ggplot(aes(y = mean_mpa, 
             x = lwa_g_cm2_new, 
             color = species, 
             shape = time,
  ...
  geom_point() +
    ...
    facet_wrap(~week*date, scales = "free")
  )
...
rwc_df0 <- rwc_df_raw %>% 
  ...
  mutate(date = case_when(
   # date %in% c("2022-02-28", "2022-03-03") ~ as.Date("2022-02-28"), 
    date %in% c("2022-04-11", "2022-04-12", "2022-04-14") ~ as.Date("2022-04-12"), 
    date %in% c("2022-05-03", "2022-05-04", "2022-05-09") ~ as.Date("2022-05-03"),
    date %in% c("2022-05-24", "2022-05-25", "2022-05-27") ~ as.Date("2022-05-25"),
    date %in% c("2022-07-27", "2022-07-28") ~ as.Date("2022-07-27"),
    date %in% c("2022-09-12", "2022-09-15") ~ as.Date("2022-09-15"),
    TRUE ~ as.Date(date)
  )) %>% 
  mutate(year_rwc = case_when(
    date %in% c("2022-04-11") ~ 1, #assume we didn't know 
    year %in% c(NA_real_) ~ 0, #assume no year listed is year 0
    year %in% c(3) ~ 1, #assume year 3 is just year 1
    TRUE ~ as.numeric(year)
  )) %>% 
  mutate(week_rwc_link = case_when(
    week %in% c(22) ~ 21, #no mpas for week 22 (prob becuase of week split in week function), so make that 21
    TRUE ~ as.numeric(week))) %>% 
  group_by(tree, date, type, year_rwc) %>% 
#remove absurd values -- issue with wet leaves having drops on them? 
  ...
  select(-date) %>% 
  distinct() %>% 
  select(mean_swc_per_dry_g, mean_swc_g, tree, date_rwc, year_rwc , type_rwc, week_rwc_link,
         ) %>% 
  as.data.frame() 
  
#MERGE
rwc_df <- merge(wp_wc_df3,rwc_df0, by = c("tree", "week_rwc_link"),
                    all = T
                    ) %>% 
  ...
  drop_na(species) %>% 
 # filter(!(mean_swc_per_dry_g > 2 & species %in% c("live oak"))) %>% #Doing this removes a whole date, not sure what was up there!
  mutate(year_rwc = as.factor(year_rwc))  %>% 
  mutate(date_rwc = case_when(
    date_rwc %in% c("2022-05-23") ~ as.Date("2022-05-25"), 
    TRUE ~ as.Date(date_rwc)
  )) %>% 
  mutate(year_rwc = case_when(
    date_rwc %in% c("2022-03-15") & species %in% c("live oak") ~ "1", #assume we didn't know 
    year_rwc %in% c(NA_real_) ~ "0", #assume no year listed is year 0
    year_rwc %in% c("3") ~ "1", #assume year 3 is just year 1
    species %in% c("blue oak") ~ "0",
    TRUE ~ as.factor(year_rwc)
  )) 

write_csv(rwc_df, here::here("processed-data", "rwc_2022.csv"))

wp_wc_rwc_df_all <- rwc_df


plotly::ggplotly(wp_wc_rwc_df_all %>% 
                   filter(species == "live oak"
                          #week %in% c(12, 14, 15)
                          ) %>% 
                   group_by(tree, time, date_wp) %>% 
...
wp_wc_rwc_alas_df1 <- merge(wp_wc_rwc_df_all, alas_df_means, by = c("week_alas_link", "tree", "type"), 
  all = T) %>% 
...
```
```{r}
plotly::ggplotly(wp_wc_rwc_alas_df1 %>% 
                   filter(species == "live oak"
                          #week %in% c(12, 14, 15)
                          ) %>% 
                   group_by(tree, time, date_wp) %>% 
...
wp_wc_rwc_alas_df <- merge(trees_sites_df, wp_wc_rwc_alas_df1, all = T) %>% 
  ...
  mutate(
    ...
    week = case_when(
      ...
      week %in% c(11) ~ 10, #or where we seem to have collected some predawns one day and some middays another day
  ...
  dplyr::group_by(tree, week, time, type_rwc) %>% #combine years to get a year mean
...
#issues found later on in analysis: 
  ...
  filter(!(tree %in% 2012)) %>% #this tree doesn't actually exist
...
#just live oaks: 
analysis_quag_df <- analysis_bothspp_df %>% 
  filter(species %in% c("live oak"))
  
write.csv(analysis_quag_df, here("processed-data", paste0("analysis_quag",datver,".csv"))) 
```

```{r}
plotly::ggplotly(analysis_bothspp_df %>% 
                   filter(species == "live oak", 
                          #week %in% c(12, 14, 15)
                          ) %>% 
  mutate(tree_num = as.numeric(tree)) %>% 
...
lwc_counts_paired <- analysis_df %>% 
  drop_na(lwc_mean) %>% 
  select(lwc_mean, tree, time, species) %>% 
  group_by(tree, week) %>% 
...
lwa_counts_paired <- analysis_df %>% 
  drop_na(lwa_g_cm2_new) %>% 
  select(lwa_g_cm2_new, tree, time, species) %>% 
  group_by(tree, week) %>% 
  filter(all(c("pd", "md") %in% time)) %>%
  ungroup() %>% 
  distinct() %>% 
...
weeks_cwc <- cwc_raw_mpa %>% 
  select(date_shift, week_shift, week_cwc_link) %>% 
  distinct()
...
weeks_cwc <- mpa_cwc_all %>% 
  select(date_shift, date_wp, week_shift, week) %>% 
  distinct()

write.csv(mpa_cwc_all, here("processed-data", paste0("cwc_analysis",datver,".csv")))
```

For figuring out WTF is up with the dates: 

```{r}
mpa_cwc_dates <- merge(wp_df%>% mutate(date = date_wp), 
                       cwc_raw %>% mutate(date = date_shift), all = T
                       )

write.csv(mpa_cwc_dates, here("processed-data", paste0("mpa_cwc_dates",datver,".csv")))
```
...
wc_qudo_df%>% 
  ggplot(aes(x = week, y = water_potential, color = water_potential))+
...
#   filter(time == "md") %>% 
  ...
  geom_point(aes(x = date, 
             y = 0, 
             color = mpa),
             data = mpa_cwc_dates %>% drop_na(mpa) %>% filter(time == "md"), 
             size = 10,
             pch=21, 
           size=10) +
    geom_point(aes(x = date,
             y = 0),
              data = mpa_cwc_dates %>% drop_na(cwc),
           pch=21,
           size=10,
           stroke=2,
           colour="black") +
...
scale_x_date(date_labels = '%b. %d', 
             breaks = as.Date(mpa_cwc_dates$date))
timeline_plot
