...
#remove absurd values -- issue with wet leaves having drops on them? 
  ...
  as.data.frame() %>% 
    merge(wp_wc_df2, by = c("tree", "week_rwc_link"),
                    all = T
                    ) %>% 
  ...
  mutate(year_rwc = case_when(
    date %in% c("2022-03-15") ~ "1", #assume we didn't know 
...
This will have duplicates, since there are multiple years for Al:As measurements
```{r}
