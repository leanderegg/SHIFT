...
data_og <- data_quke_raw %>% 
  select(tree, week, water_potential, lwc, time_season, time, species) 
...
final_df_all %>% 
  write_csv(here::here("processed-data", "bootstrapped_df_quke_lwc.csv"))
```
...
ggsave(here("figures", "quke figures", "lma_long_quke_plot"), lma_plot, device = "jpg", width = 12, height = 2, dpi = 300)
...
ggsave(here("figures", "quke figures", "alas_long_quke_plot"), alas_plot, device = "jpg", width = 12, height = 2, dpi = 300)
```
...
ggsave(here("figures", "quke figures", "nice_hydration_layered"), nice_hydration_layered, device = "jpg", width = 6, height = 4, dpi = 300)
```
...
ggsave(here("figures", "quke figures", "nice_hydration_slopes"), hyd_slopes, device = "jpg", width = 5, height = 2.5, dpi = 300)
```
...
ggsave(here("figures", "quke figures", "nice_hydration_ints"), hyd_ints, device = "jpg", width = 5, height = 2.5, dpi = 300)
```
...
ggsave(here("figures", "quke figures", "nice_hydration_slopes_bs_inds"), hyd_slopes_bs_inds, device = "jpg", width = 5, height = 2.5, dpi = 300)
```
...
ggsave(here("figures", "quke figures", "nice_space"), space_nice, device = "jpg", width = 6, height = 4, dpi = 300)
```
...
ggsave(here("figures", "quke figures", "nice_space_centered"), space_nice_centered, device = "jpg", width = 6, height = 4, dpi = 300)
```
...
ggsave(here("figures", "quke figures", "nice_time"), time, device = "jpg", width = 4, height = 2, dpi = 300)
```
...
time_mod <- lmer(water_potential ~ lwc_mean + (1|week) + (lwc_mean|tree), data = wc_quke_md_df)
#time_mod <- lmer(water_potential ~ lwc_mean + (lwc_mean|tree), data = wc_quke_md_df)
 
#with separate periods: 
time_season_mod <- lmer(water_potential ~ lwc_mean + time_season + (1|week) + (lwc_mean|tree), data = wc_quke_md_df)

time_season_mod_int <- lmer(water_potential ~ lwc_mean*time_season + (1|week) + (lwc_mean|tree), data = wc_quke_md_df)
...
lin_time_mod <- lmer(water_potential ~ lwc_mean + (1|week) + (lwc_mean|tree), data = lin_wc_quke_md_df)
...
bef_time_mod <- lmer(water_potential ~ lwc_mean + (1|week) + (lwc_mean|tree), data = bef_wc_quke_md_df, REML = T)
...
ggsave(here("figures", "quke figures", "time_summary_slopes"), time_summary, device = "jpg", width = 2.5, height = 3, dpi = 300)
...
ggsave(here("figures", "quke figures", "time_summary_slopes_boot_tree"), time_summary_bs_tree, device = "jpg", width = 2.5, height = 3, dpi = 300)
```
...
write_csv(lwc_mods, here::here("processed-data", "lwc_quke_mods.csv"))
```
...
```{r}
space_mod_4 <- lmer(water_potential ~ lwc + (lwc|week) + (1|tree), data =  data_og, REML = T)
...
ggsave(here("figures", "quke figures", "pv_slope_plot_quke"), pv_slope, device = "jpg", width = 1, height = 3, dpi = 300)
```
...
ggsave(here("figures", "quke figures", "combined_plots_all_quke"), combined_plots_all, device = "jpg", width = 10, height = 9, dpi = 300)
```
