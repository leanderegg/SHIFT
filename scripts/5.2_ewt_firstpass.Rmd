...
ewt_lwc_raw <- read_csv(here("processed-data", paste0("LWC_LFM_WP_dates",datver,".csv")), show_col_types = F) %>% 
  mutate(lwc_mean = lwc_mean_combined) %>% 
  filter(lwc_mean > 0, 
         lwc_mean < 3, 
         time == "md") %>% 
  select(tree, week, lfm_y0_percent, mpa_mean, lwc_mean, rwc_percent_y0)
