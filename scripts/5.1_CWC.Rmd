...
cwc_lwc_df <- merge(cwc_raw, cwc_lwc_raw, by = c("tree", "week"), all.y = T) %>% 
  distinct() %>% 
  # filter(cwc > -1, 
  #        #species == "blue oak"
  #        ) %>% 
  select(tree, week, date, cwc, site, time, mpa_mean, lwc_mean, lma_g_cm2, lwa_g_cm2, water_potential, time_season) %>% 
  distinct() %>% 
  filter(cwc > -100)
```
...
```{r}
mpa_cwc <- cwc_lwc_df %>% 
  ggplot(aes(y = mpa_mean, x = cwc, color = site)) +
  geom_point() +
  theme(legend.position = "none") +
  geom_smooth(method = "lm", se = F)

lwc_cwc <- cwc_lwc_df %>% 
  ggplot(aes(y = lwc_mean, x = cwc, color = site)) +
  geom_point()+
  theme(legend.position = "none")+
  geom_smooth(method = "lm", se = F)

lwa_cwc <- cwc_lwc_df %>% 
  ggplot(aes(y = lwa_g_cm2, x = cwc, color = site)) +
  geom_point()+
  theme(legend.position = "none") +
  geom_smooth(method = "lm", se = F)

lma_cwc <- cwc_lwc_df %>% 
  ggplot(aes(y = lma_g_cm2, x = cwc, color = site)) +
  geom_point()+
  geom_smooth(method = "lm", se = F)


all_cwc <- cowplot::plot_grid(mpa_cwc, lwc_cwc, lwa_cwc, lma_cwc, nrow = 2)
...
```{r}
cwc_lwc_df %>% 
  ggplot(aes(y = mpa_mean, x = cwc, color = time_season)) +
  geom_point()+
