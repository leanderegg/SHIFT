...
#Combine: 
unique(cwc_raw$week) #cwc weeks
unique(cwc_lwc_raw$week_cwc_link) #other physiology weeks

unique(cwc_raw$tree)
unique(cwc_lwc_df$tree)

cwc_lwc_df <- merge(cwc_raw, cwc_lwc_raw, by = c("tree", "week_cwc_link"), all = T) %>% 
  distinct() %>% 
  filter(cwc > -100) %>% 
 ...
 # select(-x1, -"...1") %>% 
  ...
  mutate(alas_mean = mean(alas_cm2_per_mm2, na.rm = T),
         ...
         date_shift = date) %>% 
  select(tree, shift_id, species, week, lat, lon, time, type, date_shift, date_rwc, date_alas, date_wp, cwc, mpa_mean, lwc_mean, mean_swc_per_dry_g, mean_swc_g, rwc_lfm_percent, rwc_lwc_percent, lfm_percent, alas_mean, sla_mean, leaf_count_mean, leaf_area_mean) %>% 
   distinct() %>% 
  drop_na(time)
```
