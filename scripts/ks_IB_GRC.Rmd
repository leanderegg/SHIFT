...
ggplot(aes(y = Kl_nat/1000, x = date, color = tree)) +
  ...
  geom_smooth(method = "lm", se = T, color = "black") +
  stat_summary(fun.y="mean", geom="line", aes(group=factor(tree))) +
  ggtitle("Kleaf native over time") +
...
sapflow_03312022 <- read.csv(here("processed-data", "sapflow_03312022.csv"))
sapflow_06012022 <- read.csv(here("processed-data", "sapflow_06012022.csv"))
...
sapflow_wp <- merge(sapflow_raw, sapflow_mpa, by = c("tree", "week")) %>% 
...
```{r}
sapflow_wp %>% 
  ggplot(aes(y = pdf_normed, x = date_sapflow)) +
  geom_point(aes(color = as.factor(tree)))+ 
  geom_smooth(method = "lm", se = T, color = "black") +
  #geom_smooth(method = "loess", se = T, color = "black") +
  #stat_summary(fun.y="mean", geom="line", aes(group=factor(tree), color = tree)) +
  #ggtitle("PLC over water potential space") +
  ylab("Sapflow, normalized") +
  xlab("") + 
  #th + 
  th_colors

###NOTES: missing 4.25 and 5.25 PD and MD
```
