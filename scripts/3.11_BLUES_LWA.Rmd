...
n_sample_trees <- 43  # Adjust as needed
...
```{r}
data <- data %>% 
  filter(!(week == 21)) #this week only has 1 pair of md/pd (missing scans, need to check on that) so remove it!
...
mean_slope_all <- read_csv(here::here("processed-data", "pv_lwa_slopes.csv"))
...
combined_slopes_wide <- cowplot::plot_grid(space_slope_bs, NULL, time_summary, NULL, hyd_slopes, NULL, pv_slope,
                                   nrow = 1, 
                                  rel_widths = c(1, -.021, 1, -.02, 1, -.02, .5), 
                                  labels = c("D", "", "E", "", "F", "", "G"),
                                  label_x = .2,
                                  label_y = .98
