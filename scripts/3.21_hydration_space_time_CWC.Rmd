...
unique(wc_all_raw$week)
unique(wk_all_long$week)
```
...
time_season_mod <- lmer(water_potential ~ cwc + time_season + (1|week) + (cwc|tree), data = wc_qudo_md_df)

time_season_mod_df <- broom::tidy(time_season_mod)

#with separate periods: 
time_season_mod_int <- lmer(water_potential ~ cwc*time_season + (1|week) + (cwc|tree), data = wc_qudo_md_df)

time_season_mod_int_df <- broom::tidy(time_season_mod)

time_mod <- lmer(water_potential ~ cwc + (1|week) + (cwc|tree), data = wc_qudo_md_df)
...
lin_time_mod <- lmer(water_potential ~ cwc + (1|week) + (cwc|tree), data = lin_wc_qudo_md_df)
...
bef_time_mod <- lmer(water_potential ~ cwc + (1|week) + (cwc|tree), data = bef_wc_qudo_md_df, REML = T)
