...
```{r}
slopes_hyd_aov <- aov(slope ~ week, data = ran_df_week)
...
space_mod <- lmer(water_potential ~ lwc + (1|week) + (lwc|tree), data = space_df_nosite, REML = T)
#summary(space_mod)

space_mod_site <- lmer(water_potential ~ lwc  + (1|week) + (lwc|tree) + (1|site), data = space_df_nosite,  REML = T)
#summary(space_mod_site)
