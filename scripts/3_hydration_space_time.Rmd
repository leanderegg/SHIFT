...
```
Predawnd slopes for each individual: 
```{r}
...
```{r, echo=FALSE}
time_mod <- lmer(water_potential ~ lwc + (1|week) + (lwc|tree), data = wc_qudo_pd_df)
summary(time_mod)
...
inds<- data_frame(1:48)
inds$tree <- c(unique(wc_qudo_pd_df$tree))
