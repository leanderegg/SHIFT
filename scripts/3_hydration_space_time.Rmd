...
wc_all_raw <- read_csv(here("processed-data", paste0("lfm_rwc_alas_wp_wc_df",datver,".csv")), show_col_types = FALSE) %>%
 # wc_all_raw <-  read_csv(here::here("processed-data", "wp_wc_df.csv")) %>% 
...
Correlation between LMA and MPa (developmental) is what makes LWA ~ MPa relationship non-existant (kinda) 

```{r}
wc_qudo_df  %>% #filter(lma_g_cm2>0.005 & lma_g_cm2 < 0.023) %>%
  group_by(tree, week, time) %>% 
  mutate(lma_g_cm2 = mean(lma_g_cm2, na.rm = T)) %>% 
  filter(lma_g_cm2 < .023) %>% 
  ...
  filter(time == "Predawn") %>% 
  ggplot(aes(y=water_potential, x = lma_g_cm2, 
...
require(lmerTest)
summary(lmer(lma_g_cm2~water_potential + time + (1|tree), wc_qudo_df ))
r.squaredGLMM(lmer(lma_g_cm2~water_potential + time + (1|tree), wc_qudo_df ))
```
``` {r lma vs wc}
wc_qudo_df %>% 
  filter(lma_g_cm2>0.005 & lma_g_cm2 < 0.023) %>%
  ggplot(aes(y=lwc_mean, x = lma_g_cm2, color = time))+
  geom_point() + geom_smooth(method="lm",se=F)

```
lwc = g h20/g wet leaf
...
wc_qudo_df  %>% #filter(lma_g_cm2>0.005 & lma_g_cm2 < 0.023) %>%
  ...
  filter(lma_g_cm2 < .023) %>% 
  ggplot(aes(y=water_potential, x = alas_cm2_per_mm2, color = as.factor(tree)))+
  geom_point() + 
  geom_smooth(method="lm",se=F) +
  #facet_wrap(~time) +
  theme(legend.position = "none")
...
lma_plot <- fig_df %>% 
  ...
  geom_point(size = 3) +
  geom_line() +
  scale_x_continuous(breaks = c(10, 14, 16, 21, 29, 37)
                      # seq(10, 30, by = 1)
                     ) +
  labs(y = "LMA") +
  theme(legend.position = "none", 
...
```{r}
#Try with anova:
slopes_hyd_aov <- aov(slope ~ as.factor(week), data = ran_df_week)
summary(slopes_hyd_aov)

TukeyHSD(aov(slopes_hyd_aov), ordered=TRUE)

#Try with Likelihood ratio test:

m_wk_rand <- lmer(water_potential ~ lwc + (lwc|tree) + (lwc|week), data = wk_all )

m_nowk_rand <- lmer(water_potential ~ lwc + (lwc|tree), data = wk_all )

# LRT calculated using the loglik() function
#
G2 = -2 * logLik(m_nowk_rand) + 2 * logLik(m_wk_rand)
pchisq(as.numeric(G2), df=9, lower.tail=F)
```
...
####Slopes, bootstrapping - no inds
```{r, echo=FALSE}
hyd_slopes <- final_df_all %>% 
   filter(!week %in% c(9)) %>% 
  filter(analysis == "hydration") %>% 
  ggplot(aes(color= week)
    ) + 
  geom_point(aes(y = slope ,x = week), size =3) +
    geom_point(aes(y = upr, x = week), size = 1) +
...
space_mod <- lmer(water_potential ~ lwc_centered + (lwc_centered|week) + (1|tree), data = space_df_nosite, REML = T)
#summary(space_mod)

space_mod_site <- lmer(water_potential ~ lwc_centered  + (lwc_centered|week) + (1|tree) + (1|site), data = space_df_nosite,  REML = T)
...
space_mod <- lmer(water_potential ~ lwc + (lwc|week) + (1|tree), data = space_df_nosite, REML = T)
#summary(space_mod)

space_mod_site <- lmer(water_potential ~ lwc  + (lwc|week) + (1|tree) + (1|site), data = space_df_nosite,  REML = T)
#summary(space_mod_site)
...
```{r, echo=FALSE}
space_mod_4 <- lmer(water_potential ~ lwc + (lwc|week) + (1|tree) + (1|site), data =  wc_wk_centered_df, REML = T)
...
space_mod <- lmer(water_potential ~ lwc + (1|week) + (lwc|tree), data = space_df_nosite, REML = F)
#summary(space_mod)
AIC(space_mod)

space_mod_site <- lmer(water_potential ~ lwc  + (1|week) + (lwc|tree) + (1|site), data = space_df_nosite,  REML = F)
...
#time_mod <- lmer(water_potential ~ lwc + (lwc|tree), data = wc_qudo_md_df)
...
lin_time_mod <- lmer(water_potential ~ lwc + (1|week) + (lwc|tree), data = lin_wc_qudo_md_df)
...
bef_time_mod <- lmer(water_potential ~ lwc + (1|week) + (lwc|tree), data = bef_wc_qudo_md_df, REML = T)
...
```
###Slopes, from boostrapping - no inds

```{r}
time_summary_bs <- final_df_all %>% 
   filter(!week %in% c(9)) %>% 
  filter(analysis == "time") %>% 
   mutate(week = as.factor(week), 
          time_season = time_period) %>% 
ggplot() + 
  geom_point(aes(y = slope, x = time_season), size =3) +
    geom_point(aes(y = upr, x = time_season), size = 1) +
  geom_point(aes(y = lwr, x = time_season), size = 1) +
  ...
  geom_segment(aes(y= lwr, 
                   ...
                   x = time_season, 
                   xend = time_season)) +
  labs(y = "BS Slope and CI, no ind. tree", 
       x = "Time")
time_summary_bs


ggsave(here("figures", "time_summary_slopes_boot"), time_summary_bs, device = "jpg", width = 2.5, height = 3, dpi = 300)
```
