...
goodbye indvidual leaves -- reps are confusing!
...
wc_df <- read_csv(here("processed-data", paste0("wc_alldates_",datver,".csv"))) %>% #contains LWC data
  ...
  mutate(timing = case_when(
    ...
    week %in% c(15, 16, 17, 18, 19, 20, 21, 29, 33, 37) ~ "late"
  ))%>% 
  mutate(date_wc = date) %>% 
  #select(-date) %>% 
  #drop_na(date_wc)  %>% 
...
wp_wc_df <- merge(wp_df, wc_df, 
                  by = c("tree","plot", "site", "species", "time", "week", "rep"), 
                  all = T) %>% 
  filter(species != "ARCA", 
         species != "LEU") %>% 
...
long <- wp_wc_df %>% 
  pivot_longer(cols = c(lwc_leaf_mean, lwc_bulk_mean),
    names_to = "type", 
               values_to = "lwc") %>% 
  select(tree, species, time, week, mpa_mean, date_wp, type, lwc, site) %>% 
  distinct() %>% 
  group_by(tree, time, week, species, site) %>% 
  summarise(lwc_mean_combined = mean(lwc, na.rm = T), 
            mpa_mean = mpa_mean, 
            date_wp = date_wp
            ) %>% 
  #select(-type) %>% 
  distinct() %>% 
  mutate(lwc_bulk = lwc_mean_combined, 
         lwc_mean = lwc_mean_combined)
...
wp_wc_rwc_df <- merge(long, rwc_df_leaves_y0,
                    by = c("tree",
                           "week"
                          # "rep"
                           ),
...
wp_wc_rwc_lfm_df_wide_trees_leaves <- merge(trees_sites_df, wp_wc_rwc_lfm_df_wide_leaves, 
                                     by = c("tree")) %>% 
 # select(-rep) %>% 
  distinct()
...
wp_wc_rwc_lfm_df_wide_trees_stem <- merge(trees_sites_df, wp_wc_rwc_lfm_df_wide_stem, 
                                     by = c("tree")) %>% 
 # select(-rep) %>% 
  distinct()
...
all_water_df <- bind_rows(wp_wc_rwc_lfm_df_wide_trees_leaves, wp_wc_rwc_lfm_df_wide_trees_stem) %>% 
  mutate(rwc_percent_y0 = ((lwc_mean_combined/mean_swc_per_dry_g_y0)*100), 
...
#wp_wc_rwc_lfm_df_wide_trees_leaves %>%
  all_water_df %>% 
 # filter(species == "blue oak") %>% 
  select(week, time, tree, lwc_mean_combined, species) %>% 
  distinct() %>% 
  drop_na(lwc_mean_combined) %>% 
