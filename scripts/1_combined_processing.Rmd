...
LFM data, to be used as write-in LWC when missing: 
```{r}
...
lfm_merge_lwc <- lfm_df %>% 
  filter(type == "leaf") %>% 
  select(tree, week, lfm_wet_per_dry_g, time) %>% 
  group_by(tree, week, time) %>% 
  mutate(lfm = mean(lfm_wet_per_dry_g, na.rm = T)) %>% 
  select(tree, lfm, time, week) %>% 
  distinct()
```
...
wp_wc_df <- merge(wp_df, wc_df, 
  ...
  filter(species != "ARCA", 
         species != "LEU") %>% 
  distinct() %>% 
#remove weird values before we combine leaves and bulk:
...
#combine reps to get mean mpa and lwc per tree; combine bulk and separate lwc: 
  ...
  mutate(mpa_mean = mean(mpa, na.rm = T), 
        ...
        lwc_bulk_leaf_diff = lwc_leaf_mean-lwc_bulk_mean) %>%
  ungroup() %>% 
  dplyr::select(-rep) %>%
  distinct() %>% 
  ...
  distinct() %>% 
#since we didn't collect RWC every week, create a column we can use to link RWC to all of the closest LWC weeks: 
...
#since we didn't collect alas every week, create a column we can use to link LMA to all of the closest LWC weeks: 
...
#remove absurd values -- issue with wet leaves? 
...
unique(alas_df_means$week_alas_link)
```

```{r}
#merge alas_df_means with the df with wp, wc, and rwc-related values: 
...
wp_wc_rwc_alas_df <- merge(trees_sites_df, wp_wc_rwc_alas_df1, by = c("tree"), all = T) %>% 
  ...
  #filter(!week == 9)%>% 
  group_by(tree, week) %>% 
  fill(alas_cm2_per_mm2, date_wp, leaf_count,.direction = "downup") %>% 
  ungroup() %>% 
  distinct() %>% 
  drop_na(time) 
```
...
analysis_df <- wp_wc_rwc_alas_df %>% 
  ...
  select(week, time, tree, plot, site, species, 
        mpa_mean, lwc_mean, lma_g_cm2_mean, 
         ldmc_mean, alas_cm2_per_mm2_mean, lwa_g_cm2_mean, leaf_count, swc_per_dry_g_mean, rwc_mean, date_wp, week_alas_link) %>% 
  distinct() %>% 
  ...
  mutate(alas_cm2_per_mm2 = case_when(
    alas_cm2_per_mm2 > 60 ~ NA_real_, #remove alas outliers
...
cwc_analysis_df <-  cwc_lwc_all_df %>% 
   select(tree, site, species, week, lat, lon, time, date_shift,date_wp, cwc, lwc_mean, lma_g_cm2, alas_cm2_per_mm2, lwa_g_cm2, leaf_count, week_cwc_link, water_potential, time_season) %>%
   distinct() %>% 
  mutate(week = case_when(
             week %in% NA_real_ ~ week_cwc_link, 
             TRUE ~ as.numeric(week)
           ))%>% 
  group_by(tree, week) %>% 
  mutate(cwc = mean(cwc)) %>% 
...
```{r}
lfm_rwc_alas_wp_wc_df <- merge(wp_wc_rwc_alas_df, lfm_df, by = c("tree", "week", "time", "type", "year"), 
                               all = T) %>% 
  filter(tree > 100) %>% 
  distinct()  %>% 
  dplyr::group_by(tree, time, week) %>%
