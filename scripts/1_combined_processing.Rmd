...
# wc_lfm_df <- merge(wc_df, lfm_merge_lwc) %>% 
#   filter(tree > 1000) 
...
alas_df_means <- alas_df %>% 
  ...
  select(-branch, -leaf_count, -lwm_g, -l_cm, -d_cm, -area_cm2) %>% 
  distinct()
```


```{r}
wp_wc_rwc_alas_df1 <- merge(wp_wc_rwc_df, alas_df_means, by = c("week", "tree", "year"), 
  all = T) %>% 
  ...
  distinct() %>% 
  filter(type == "leaf", #remove those we're not interested in
         !(year == 3 | year == 2))
...
wp_wc_rwc_alas_df %>% 
  select(lma_g_cm2, mean_leaf_count, alas_cm2_per_mm2, week, species) %>% 
  distinct() %>% 
  filter(lma_g_cm2 < .04, 
         #time == "pd"
         ) %>% 
  ggplot(aes(y = alas_cm2_per_mm2, x = mean_leaf_count, color = species)) +
  geom_point() +
 # facet_wrap(~time) +
  geom_smooth(method = "lm")
```
...
wp_wc_rwc_alas_df %>% 
  filter(time == "pd", 
         lma_g_cm2 < .04) %>% 
  ggplot(aes(x = mpa_mean, y = lma_g_cm2, color = species)) +
  geom_point()
```
