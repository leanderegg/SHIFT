...
rwc_df_spp <- merge(rwc_df, trees_sites_df, all = T)
...
alas_df_raw <- read_csv(here::here("processed-data", paste0("alas_leaf_area_df_week",datver,".csv")), show_col_types = F) %>% 
  mutate(tree = tree_id) %>% 
  select(tree, week, branch, year, lwm_g, ldm_g, l_cm, d_cm, area_cm2, lma_g_cm2, alas_cm2_per_mm2, sla_cm_g) %>% 
...
wp_wc_rwc_alas_df <- merge(wp_wc_rwc_df, alas_df_raw, by = c("week", "tree"), all = T) %>% 
  mutate(lwa_g_cm2 = lwc_mean * lma_g_cm2) %>% 
  select(-timing) %>% 
  distinct()
```

#FINAL: What is CWC df
```{r}
write.csv(wp_wc_rwc_alas_df, here("processed-data", paste0("wp_wc_rwc_",datver,".csv"))) 

unique(wp_wc_rwc_alas_df$date_rwc
       )
```
...
```{r}
alas_all_water_df <- merge(all_water_df, alas_df_raw, by = c("week", "tree"), all = T) %>% 
  mutate(lwa_g_cm2 = lwc_mean * lma_g_cm2)
```

This dataset is just for the what is CWC story, i.e., removed a lot of extra rows with the LFM data (i.e. stems, different years, etc)
...
all_water_df_alas <- alas_all_water_df %>% 
  ...
  select(week, tree, time, plot, site, species, mpa_mean, lwc_mean, mean_swc_per_dry_g_y0, mean_swc_g_y0, lfm_y0_percent, lfm_y1_percent, lfm_both_percent, rwc_percent_y0, lwm_g, ldm_g, area_cm2, lma_g_cm2, alas_cm2_per_mm2, sla_cm_g, lwa_g_cm2, date_wp) %>% 
  distinct() %>% 
  filter(tree > 1000)
```
...
```{r}
# all_water_df_alas %>% 
#   mutate(lwc_lfm = (lwm_g - ldm_g)/ldm_g*100) %>% 
#   filter(lwc_lfm < 300, lwc_lfm > 0) %>% 
#   ggplot(aes(y = lfm_y1_percent, x = lwc_lfm, color = species)) +
#   geom_point() +
#   geom_abline() +
#   coord_equal()
#   
...
sedgwick <- all_water_df_alas %>% 
  ...
  select(date, week, tree, time, plot, site, species, 7:19)
