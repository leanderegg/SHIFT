...
leaf_number <- read_csv(here(here(dataversion,"alas_leaf_number.csv")), show_col_types = FALSE) %>% 
  clean_names() %>% 
...
wp_wc_rwc_alas_df <- merge(wp_wc_rwc_df, alas_df, by = c("week", "tree"), all = T) %>% 
...
all_water_df_alas <- alas_all_water_df %>% 
  ...
  select(week, tree, time, plot, site, species, mpa_mean, lwc_mean, mean_swc_per_dry_g_y0, mean_swc_g_y0, lfm_y0_percent, lfm_y1_percent, lfm_both_percent, rwc_percent_y0, lwm_g, ldm_g, area_cm2, lma_g_cm2, alas_cm2_per_mm2, leaf_count, sla_cm_g, lwa_g_cm2, date_wp) %>% 
  distinct() %>% 
  filter(tree > 1000)
```
