...
wc_all_raw <- read_csv(here("processed-data", paste0("wp_wc_rwc_",datver,".csv")), show_col_types = F) %>%   #mutate(lwc_mean = lwc_mean_combined) %>% 
 # here("processed-data", paste0("LWC_LFM_WP_dates",datver,".csv")))
  filter(lwc_mean > 0, 
         lwc_mean < 3)


wc_all_raw2 <- read_csv(here("processed-data", paste0("LWC_LFM_WP_dates",datver,".csv")), show_col_types = F) %>%   #mutate(lwc_mean = lwc_mean_combined) %>% 
 # here("processed-data", paste0("LWC_LFM_WP_dates",datver,".csv")))
  filter(lwc_mean > 0, 
         lwc_mean < 3)

wc_all_lma <- wc_all_raw %>% 
  filter( #week > 15, 
         species == "blue oak", 
         lwc_mean < 10) %>% 
  group_by(tree, week, time, lwc_mean, site) %>% 
  mutate(lma = mean(lma_g_cm2, na.rm = T)) 
