...
```{r}
m0 <- lm(lwc ~ 1 + (1|tree), data = qudo_wc_linear)
m1 <- lm(lwc ~ week + (1|tree), data = qudo_wc_linear)
m2 <- lm(lwc ~ water_potential + (1|tree), data = qudo_wc_linear)
m3 <- lm(lwc ~ week + water_potential + (1|tree), data = qudo_wc_linear)
...
mod_all_df1 <- mod_all_df %>% 
  ...
  mutate(week_effect = m3 - m2, #week + mpa mod - mpa mod
         ...
         week_mpa_effect = m3 - m0, #week + mpa mod - null mod
         all_effect = m2 + m1) #sum of week and mpa effects alone
mod_all_df1

#for plotting (maybe?)
mod_all_long <- mod_all_df1 %>% 
  select(week_effect, mpa_effect, week_mpa_effect, all_effect) %>% 
  pivot_longer(cols = c(week_effect, mpa_effect, week_mpa_effect, all_effect), 
    names_to = "effect") %>% 
  distinct() %>% 
  mutate(analysis = case_when(
    effect %in% c("week_effect", "mpa_effect", "week_mpa_effect") ~ "Var. Decomp", 
