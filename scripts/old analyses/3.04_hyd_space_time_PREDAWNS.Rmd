...
wc_qudo_df  %>% #filter(lma_g_cm2>0.005 & lma_g_cm2 < 0.023) %>%
  ...
  filter(time == "Midday") %>% 
  ggplot(aes(y=water_potential, x = lma_g_cm2, color = as.factor(tree)))+
  geom_point() + 
...
require(lmerTest)
summary(lmer(lma_g_cm2~water_potential + time + (1|tree), wc_qudo_lwa_df ))
r.squaredGLMM(lmer(lma_g_cm2~water_potential + time + (1|tree), wc_qudo_lwa_df ))
```
``` {r lma vs wc}
wc_qudo_df %>% 
...
timeline_df <- wc_qudo_df %>% 
 # select(week, date_wp, time) %>% 
  ...
  select(date_new, week, water_potential) %>% 
  distinct()
...
timeline_plot <- timeline_df %>% 
 ...
 drop_na(water_potential) %>% 
 # filter(time == "pd") %>% 
  ggplot(aes(x = date_new, y = 0, color = water_potential))+
  geom_hline(yintercept = 0) +
  geom_point(size = 10) +
  color_grad_rev+
  ylim(-.25, +.25)  + 
...
time_mod <- lmer(water_potential ~ lwc + (1|week) + (lwc|tree) + (1|site), data = wc_qudo_md_df)
...
lin_time_mod <- lmer(water_potential ~ lwc + (1|week) + (lwc|tree) + (1|site), data = lin_wc_qudo_md_df)
...
bef_time_mod <- lmer(water_potential ~ lwc + (1|week) + (lwc|tree) + (1|site), data = bef_wc_qudo_md_df, REML = T)
