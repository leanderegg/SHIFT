...
#when there are bigish differences, replace the combined bulk+leaf with just the bulk; otherwise take the mean of both. 
  ...
  #select(-dm_leaf_g, -wm_leaf_g) %>% 
  distinct() 

#some of these have two values for bulk masses
```
...
wc_lwa_long <- wc_df %>% 
  ...
  #select(-rep, -dm_bulk_g, -dm_leaf_g, -lwc_bulk, -lwc_leaf, -wm_bulk_g, -wm_leaf_g) %>% 
   select(-dm_bulk_g, -dm_leaf_g, -lwc_bulk, -lwc_leaf, -wm_bulk_g, -wm_leaf_g) %>% 
...
wc_lma_long <- wc_df %>% 
  select(-lwc_bulk, -lwc_leaf, -wm_bulk_g, -wm_leaf_g) %>% 
...
wc_df_lwa <- merge(wc_long_lwa_all2, lwa_new_df, 
...
plotly::ggplotly(wc_df_lwa  %>%  #masses look correct
                 #filter(week == 33) %>% 
                   drop_na(sum_area_cm2) %>% 
...
lwa_df <- wc_df_lwa %>% 
  ...
  mutate(lma_g_cm2_new = case_when(
    lwc_wrong %in% c("off") ~ NA_real_,
    tree %in% c(2383, 2384) & week %in% c(37) ~ NA_real_,
    lma_g_cm2_new > 0.09 ~ NA_real_, #to remove weirdly high lmas from week 21
    lma_g_cm2_new > 100 ~ NA_real_, #to remove infinities
...
#get rid of outliers identified above: 
wp_wc_lwa_df2 <- wp_wc_lwa_df1 %>% 
  mutate(lwa_g_cm2_new = case_when(
...
#since we didn't collect alas every week, create a column we can use to link LMA to all of the closest LWC weeks (should probably linearly interpolate at some point):
  ...
  filter(!if_all(c(mean_mpa), is.na)) %>% 
...
#write.csv(wp_wc_lwa_df, here::here("processed-data", "wp_wc_df.csv"))
```
...
#remove absurd values -- issue with wet leaves having drops on them? 
  ...
  select(-date) %>% 
  distinct() %>% 
  select(mean_swc_per_dry_g, mean_swc_g, tree, year,type_rwc, week_rwc_link,
         ) %>% 
  as.data.frame() %>% 
  distinct()
```
...
```{r}
wp_wc_rwc_lwa_df_all <- merge(wp_wc_lwa_df, rwc_df,
...
wp_wc_rwc_df <- wp_wc_rwc_lwa_df_all %>% 
  filter(!(year_rwc %in% c(1) & type_rwc %in% c("leaf")), 
        !(type_rwc %in% c("stem")))

###SWC Figure: 

wp_wc_rwc_lwa_df_all %>% 
...
wp_wc_rwc_lwa_df_all %>% 
  ...
  # )) %>% 
  ggplot(aes(y = mean_swc_per_dry_g, x = mean_mpa, color = as.factor(week))) +
  geom_jitter(alpha = .3) +
 # facet_wrap(~growth_words) +
  #geom_smooth(method = "lm", se = F)+
  color_many +
...
#Next, Merge with the OG df: merge alas_df_means with the df with wp, wc, lwa, and rwc-related values (NOTE: After this merge, SWC, LMA, and Al:As - and other related metrics -  will be duplicated since they need to be in rows for both predawn and midday to calculate inferred LWA)

wp_wc_rwc_alas_df1 <- merge(wp_wc_lwa_rwc_df, alas_df_means, by = c("week_alas_link", "tree", "type"), 
  all = T) %>% 
...
lwa_data <- all_data_new_lwa %>% 
 ...
 # select(-lwa_g_cm2) %>% 
  distinct() %>% 
  mutate(mismatch = lma_g_cm2 - lma_g_cm2_new)
...
wp_wc_rwc_alas_df <- merge(trees_sites_df, all_data_new_lwa, all = T) %>% 
  ...
  ungroup() %>% 
    group_by(tree, week, time, date_wp) %>% 
  fill(lma_g_cm2_new, lwa_g_cm2_new ,.direction = "downup") %>% 
  ungroup() %>% 
  distinct() %>% 
  drop_na(time) 
```
...
####Dataset with WP + WC + RWC + LWA, both means and actual values: 
```{r}
...
####**Dataset for analysis of LWC, MPA, LWA:
...
lfm_df_all <- read_csv(here("processed-data", paste0("lfm_alldates_",datver,".csv"))) %>% 
  select(-'...1') %>% 
  mutate(year_alas = year) #rename year column so that it plays night with the main df (we want the alas year and the LFM years to line up, so year_alas and year_lfm area just the same)
