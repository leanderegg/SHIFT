...
###Just last week of April and May:
```{r, eval =}
###dcwc leafout: 
data <- filtered_data  %>% 
  filter(week %in% c(17:21)) %>% 
  distinct() %>% 
  drop_na(cwc) %>% 
  group_by(tree) %>% 
  mutate(COUNT = n()) %>% 
...
```
#2.25. FIGURES: All dCWC relationships visualized, no Bonferroni:
```{r}
...
#2.75. FIGURES: All dCWC relationships visualized, Bonferroni:
```{r}
fig_data <- merge(dcwc_alldata %>% select(-analysis), 
                  sig_mods_all_sig, by = c("date_wp", "date_new", "time"), all.y = T) %>% 
  mutate(r_squared = round(r_squared, 3)) %>% 
  filter(!(variable == "lma_g_cm2")) %>% 
  ungroup() %>% 
  distinct() %>% 
