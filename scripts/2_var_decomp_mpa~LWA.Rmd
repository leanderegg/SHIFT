---
title: "Establishing relationships between lwa_g_cm2 and MPa"
author: "Indra Boving"
date: "11/8/2022"
...
m2 <- lmer(water_potential ~ lwa_g_cm2 + (1|tree), data = qudo_wc_all)
m3 <- lmer(water_potential ~ week + lwa_g_cm2 + (1|tree), data = qudo_wc_all)
...
mod_all_df_full <- mod_all_df1_full %>% 
  pivot_wider(names_from = mods, 
              values_from = r2m) %>% 
  mutate(
    #other_effects =  m3 - week_effect - lwa_g_cm2_effect,
    week_effect = m3 - m2, #week + mpa mod - mpa mod
         lwa_g_cm2_effect = m3 - m1, #week + mpa mod - week mod
         week_lwa_g_cm2_effect = m3, #week + mpa mod 
    other_effects =  m3 - week_effect - lwa_g_cm2_effect,
...
mod_all_long_full <- mod_all_df_full %>% 
  select(week_effect, lwa_g_cm2_effect, week_lwa_g_cm2_effect, other_effects) %>% 
  pivot_longer(cols = c(week_effect, lwa_g_cm2_effect, week_lwa_g_cm2_effect, other_effects), 
    names_to = "effect") %>% 
  distinct() %>% 
  mutate(analysis = case_when(
    effect %in% c("week_effect", "lwa_g_cm2_effect", "other_effects") ~ "Var. Decomp", 
    effect %in% c("week_lwa_g_cm2_effect") ~ "Full Mod.", 
...
qudo_wc_full <- wc_all_raw %>% 
   filter(#week > 15, 
         species == "blue oak", 
         lwa_g_cm2 < 10) %>% 
  group_by(tree, week, time, lwa_g_cm2) %>%  
  mutate(section = "full") %>% 
  select(section, species, week, water_potential, lwa_g_cm2, site)
...
mvar <- lmer(water_potential ~ lwa_g_cm2 + (1|site/tree) + (1|week), data=qudo_wc_full)

r2m <- r.squaredGLMM(mvar)[1] # plust the marginal R2 of lwa_g_cm2
#r2m 
...
mvar_lin <- lmer(water_potential ~ lwa_g_cm2 + (1|site/tree) + (1|week), data=qudo_wc_linear)

r2m <- r.squaredGLMM(mvar_lin)[1] # plust the marginal R2 of lwa_g_cm2
...
ggsave(here("figures", "vdc2_plot"), vdc2_plot, device = "jpg", width = 5.5, height = 7, dpi = 300)
```
...
ggsave(here("figures", "vardecomp_lma_plot"), vardecomp_lma_plot, device = "jpg", width = 12, height = 5, dpi = 300)
```
