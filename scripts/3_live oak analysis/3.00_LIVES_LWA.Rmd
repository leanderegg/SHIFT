...
```{r}
data_quke_raw <- read_csv(here::here("processed-data", paste0("analysis_bothspp",datver,".csv")), show_col_types = FALSE) %>% 
...
final_df_all %>% 
  write_csv(here::here("processed-data", "bootstrapped_df_quke_lwa_g_cm2_new.csv"))
```
...
ggsave(here("figures", "quke figures", "LWA", "lma_long_quke_plot"), lma_plot, device = "jpg", width = 12, height = 2, dpi = 300)
...
ggsave(here("figures", "quke figures", "LWA", "alas_long_quke_plot"), alas_plot, device = "jpg", width = 12, height = 2, dpi = 300)
```
...
ggsave(here("figures", "quke figures", "LWA", "nice_hydration_layered"), nice_hydration_layered, device = "jpg", width = 6, height = 4, dpi = 300)
```
...
ggsave(here("figures", "quke figures", "LWA", "nice_hydration_slopes"), hyd_slopes, device = "jpg", width = 5, height = 2.5, dpi = 300)
```
...
ggsave(here("figures", "quke figures", "LWA", "nice_hydration_ints"), hyd_ints, device = "jpg", width = 5, height = 2.5, dpi = 300)
```
...
ggsave(here("figures", "quke figures", "LWA", "nice_hydration_slopes_bs_inds"), hyd_slopes_bs_inds, device = "jpg", width = 5, height = 2.5, dpi = 300)
```
...
ggsave(here("figures", "quke figures", "LWA", "nice_space"), space_nice, device = "jpg", width = 6, height = 4, dpi = 300)
```
...
ggsave(here("figures", "quke figures", "LWA", "nice_space_centered"), space_nice_centered, device = "jpg", width = 6, height = 4, dpi = 300)
```
...
```{r, warning = F, eval = F}
wc_wk_centered_pd_df <- wc_quke_df %>% 
  filter(time == "pd") %>% 
  group_by(week) %>% 
...
ggsave(here("figures", "quke figures", "LWA", "nice_time"), time, device = "jpg", width = 4, height = 2, dpi = 300)
```
...
```{r}
wc_quke_md_df <- wc_quke_df %>% 
  filter(time == "md")
  
time_mod <- lmer(water_potential ~ lwa_g_cm2_new + (1|week) + (lwa_g_cm2_new|tree), data = wc_quke_md_df)
#time_mod <- lmer(water_potential ~ lwa_g_cm2_new + (lwa_g_cm2_new|tree), data = wc_quke_md_df)
 
#with separate periods: 
time_season_mod <- lmer(water_potential ~ lwa_g_cm2_new + time_season + (1|week) + (lwa_g_cm2_new|tree), data = wc_quke_md_df)

time_season_mod_int <- lmer(water_potential ~ lwa_g_cm2_new*time_season + (1|week) + (lwa_g_cm2_new|tree), data = wc_quke_md_df)
...
####Just linear: 
```{r}
lin_wc_quke_md_df <- wc_quke_df %>% 
  filter(time == "md") %>% 
  filter(time_season == "after leafout")
  
  
lin_time_mod <- lmer(water_potential ~ lwa_g_cm2_new + (1|week) + (lwa_g_cm2_new|tree), data = lin_wc_quke_md_df)
...
bef_time_mod <- lmer(water_potential ~ lwa_g_cm2_new + (1|week) + (lwa_g_cm2_new|tree), data = bef_wc_quke_md_df, REML = T)
...
ggsave(here("figures", "quke figures", "LWA", "time_summary_slopes"), time_summary, device = "jpg", width = 2.5, height = 3, dpi = 300)
...
ggsave(here("figures", "quke figures", "LWA", "time_summary_slopes_boot_tree"), time_summary_bs_tree, device = "jpg", width = 2.5, height = 3, dpi = 300)
```
...
write_csv(lwa_g_cm2_new_mods, here::here("processed-data", "lwa_g_cm2_new_quke_mods.csv"))
```
...
ggsave(here("figures", "quke figures", "LWA", "pv_slope_plot_quke"), pv_slope, device = "jpg", width = 1, height = 3, dpi = 300)
```
...
ggsave(here("figures", "quke figures", "LWA", "combined_plots_all_quke"), combined_plots_all, device = "jpg", width = 10, height = 9, dpi = 300)
```
