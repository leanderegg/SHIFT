...
final_df_all %>% 
  write_csv(here::here("processed-data", "bootstrapped_df_quke_lwa_g_cm2.csv"))
```
...
fig_df <- data_quke_raw %>% 
  select(tree, week, water_potential, lwa_g_cm2, lma_g_cm2, time_season, time, species, week_alas_link) %>% 
  distinct() %>% 
...
```{r, warning=T}
m_wk_rand <- lmer(water_potential ~ lwa_g_cm2 + (lwa_g_cm2|tree) + (lwa_g_cm2|week), data = wk_all )
```
...
m_wk_rand_nosite <- lmer(water_potential ~ lwa_g_cm2 + (lwa_g_cm2|tree) + (lwa_g_cm2|week), data = wk_all_sites, REML = T)
#m_wk_rand_nosite

m_wk_rand_site <- lmer(water_potential ~ lwa_g_cm2 + (lwa_g_cm2|tree) + (lwa_g_cm2|week) + (1|site), data = wk_all_sites, REML = T)
#m_wk_rand_site
...
m_wk_rand <- lmer(water_potential ~ lwa_g_cm2 + (lwa_g_cm2|tree) + (lwa_g_cm2|week) + (1|site), data = wk_all_sites, REML = T)
#m_wk_rand_site
```
...
lwa_g_cm2 uncentered:

```{r}
space_df_nosite <- wc_wk_centered_df %>% 
  drop_na(site)

space_mod <- lmer(water_potential ~ lwa_g_cm2 + (lwa_g_cm2|week) + (1|tree), data = space_df_nosite, REML = T)
#summary(space_mod)

space_mod_site <- lmer(water_potential ~ lwa_g_cm2  + (lwa_g_cm2|week) + (1|tree) + (1|site), data = space_df_nosite,  REML = T)
#summary(space_mod_site)
...
```{r, echo=FALSE}
space_mod_4 <- lmer(water_potential ~ lwa_g_cm2 + (lwa_g_cm2|week) + (1|tree) + (1|site), data =  wc_wk_centered_df, REML = T)
...
space_mod <- lmer(water_potential ~ lwa_g_cm2 + (1|week) + (lwa_g_cm2|tree), data = space_df_nosite, REML = F)
#summary(space_mod)
AIC(space_mod)

space_mod_site <- lmer(water_potential ~ lwa_g_cm2  + (1|week) + (lwa_g_cm2|tree) + (1|site), data = space_df_nosite,  REML = F)
...
time_mod <- lmer(water_potential ~ lwa_g_cm2 + (1|week) + (lwa_g_cm2|tree), data = wc_quke_md_df)
#time_mod <- lmer(water_potential ~ lwa_g_cm2 + (lwa_g_cm2|tree), data = wc_quke_md_df)
 
#with separate periods: 
time_season_mod <- lmer(water_potential ~ lwa_g_cm2 + time_season + (1|week) + (lwa_g_cm2|tree), data = wc_quke_md_df)

time_season_mod_int <- lmer(water_potential ~ lwa_g_cm2*time_season + (1|week) + (lwa_g_cm2|tree), data = wc_quke_md_df)
...
lin_time_mod <- lmer(water_potential ~ lwa_g_cm2 + (1|week) + (lwa_g_cm2|tree), data = lin_wc_quke_md_df)
...
bef_time_mod <- lmer(water_potential ~ lwa_g_cm2 + (1|week) + (lwa_g_cm2|tree), data = bef_wc_quke_md_df, REML = T)
...
Tidy mods
```{r}
lwa_g_cm2_mods <- bind_rows(space_mod_df <- space_mod_df %>% 
                        mutate(mod = "space"), 
  ...
  bind_rows(glance_df) %>% 
    mutate(var = "lwa_g_cm2")

lwa_g_cm2_mods

write_csv(lwa_g_cm2_mods, here::here("processed-data", "lwa_g_cm2_quke_mods.csv"))
```
...
```{r}
space_mod_4 <- lmer(water_potential ~ lwa_g_cm2 + (lwa_g_cm2|week) + (1|tree), data =  data_og, REML = T)
