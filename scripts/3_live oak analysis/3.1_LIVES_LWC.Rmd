...
m1 <- lmer(mean_mpa ~ lma_g_cm2_new + time + week + (1|tree) + (lma_g_cm2_new|tree), data = quag_lma_mpa_df, REML = F)
summary(m1)
MuMIn::r.squaredGLMM(m1) #0.8329473

m2 <- lm(mean_mpa ~ time + week + (1|tree) + (lma_g_cm2_new|tree), data = quag_lma_mpa_df, REML = F)
summary(m2)
...
- PD alone: 
```{r}
quag_lma_mpa_df_pd <- quag_lma_mpa_df %>% 
  filter(time == "pd")
#just pds: 

#just pds: 
m2 <- lmer(mean_mpa ~ lma_g_cm2_new + week + (lma_g_cm2_new|tree), data = quag_lma_mpa_df_pd, REML = F)

#m2 <- lm(mean_mpa ~ lma_g_cm2_new + week, data = quag_lma_mpa_df_pd, REML = F)
#summary(m2)
MuMIn::r.squaredGLMM(m2)


m3 <- lmer(mean_mpa ~ week + (lma_g_cm2_new|tree), data = quag_lma_mpa_df_pd, REML = F)
#summary(m3)
MuMIn::r.squaredGLMM(m3)


#does lma tell us more than week does? Or does just knowing the week tell us more? 
anova(m2, m3) #LMA does not contribute significantly to predicting MPa

m4 <- lmer(mean_mpa ~ lma_g_cm2_new + (lma_g_cm2_new|tree), data = quag_lma_mpa_df_pd, REML = F)
#summary(m4)

#Store the R-squared of the 'better' model
marginal_r2_pd <- round(MuMIn::r.squaredGLMM(m4)[1], digits = 3)
marginal_r2_pd
```

- For just MDs:
```{r}
quag_lma_mpa_df_md <- quag_lma_mpa_df %>% 
  filter(time == "md")
#just mds: 
m2 <- lmer(mean_mpa ~ lma_g_cm2_new + week + (lma_g_cm2_new|tree), data = quag_lma_mpa_df_md, REML = F)
MuMIn::r.squaredGLMM(m2)

m3 <- lmer(mean_mpa ~ week + (lma_g_cm2_new|tree), data = quag_lma_mpa_df_md, REML = F)
summary(m3)
MuMIn::r.squaredGLMM(m3)
...
m4 <- lmer(mean_mpa ~ week + (lma_g_cm2_new|tree), data = quag_lma_mpa_df_md, REML = F)

#Store the R-squared of the 'better' model
marginal_r2_md <- round(MuMIn::r.squaredGLMM(m4)[1], digits = 3)
marginal_r2_md
```
