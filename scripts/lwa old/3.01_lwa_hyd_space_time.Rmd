...
#Make wide so that we only have rows where we have matching md and pd values (for hydration analysis) 

wk_all_wide <- wc_qudo_df0 %>% 
...
wk_all_long <- merge(wk_all_long_mpa, wk_all_long_lwa_g_cm2) %>% 
 ...
 # select(-lwa_g_cm2_NA, -water_potential_NA) %>% 
  ...
  reframe(lwa_g_cm2 = mean(lwa_g_cm2, na.rm = T), 
...
```{r, warning=T}
m_wk_rand <- lmer(water_potential ~ lwa_g_cm2 + (lwa_g_cm2|tree) + (lwa_g_cm2|week), data = df_all_groups)
```

Should we include site? 

```{r, include = F}
wk_all_sites <- df_all_groups %>% mutate(site = as.factor(site)) %>% 
  drop_na(site)
...
```{r, warning = F}
wc_wk_centered_lwa_df <- wc_qudo_df0 %>% 
  filter(time == "md") %>% 
  group_by(week) %>% 
...
```{r, warning = F}
wc_wk_centered_pd_lwa_df <- wc_qudo_df0 %>% 
  filter(time == "pd") %>% 
  group_by(week) %>% 
...
```{r}
space_lwa_df_nosite <- wc_qudo_df0 %>% 
  drop_na(site)
...
Just linear: 
```{r}
lin_wc_qudo_md_lwa_df <- wc_qudo_df0 %>% 
  filter(time == "md") %>% 
  filter(time_season == "after leafout")
  
  
lin_time_mod <- lmer(water_potential ~ lwa_g_cm2 + (1|week) + (lwa_g_cm2|tree), data = lin_wc_qudo_md_lwa_df)
...
```{r}
bef_wc_qudo_md_lwa_df <- wc_qudo_df0 %>% 
  filter(time == "md") %>% 
