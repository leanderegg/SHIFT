...
wc_qudo_lwa_df<- read_csv(here("processed-data", paste0("wp_wc_rwc_",datver,".csv")), show_col_types = FALSE) %>% 
...
wc_qudo_lwa_df %>% 
  ggplot(aes(x = week, y = 0, color = week))+
...
timeline_lwa_df <- wc_qudo_lwa_df %>% 
...
timeline_plot <- timeline_lwa_df %>% 
 # filter(time == "md") %>% 
  ggplot(aes(x = date_new, y = 0, color = week))+
  geom_hline(yintercept = 0) +
  geom_point(size = 10) +
  color_grad +
  ylim(-.25, +.25)  + 
  theme(legend.position = "none", 
        ...
        axis.text.y = element_blank(),
        axis.ticks = element_blank(), 
        axis.text.x = element_text(angle = 60, vjust = 0.5, hjust=.5, size= 14)
        ) +
...
ggsave(here("figures", "timeline_plot"), timeline_plot, device = "jpg", width = 12, height = 1.5, dpi = 300)
```
...
ggsave(here("figures", "nice_hydration_lwa"), nice_hyd, device = "jpg", width = 3, height = 9, dpi = 300)
```
...
# df_grey_groups %>% 
#   ggplot() +
#   geom_point(aes(y=water_potential,x= lwa_g_cm2, group = tree, color=color)) + 
#   geom_line(aes(y=water_potential,x= lwa_g_cm2, group = tree, color=color)) + 
...
ggsave(here("figures", "nice_hydration_layered_lwa"), nice_hydration_layered, device = "jpg", width = 6, height = 4, dpi = 300)
```
...
#summary(lmer(water_potential~lwa_g_cm2 + (1|week), wc_wk_centered_lwa_df))
#scaled
...
space_mod <- lmer(water_potential ~ lwa_g_cm2_centered + (lwa_g_cm2_centered|week) + (1|tree), data = space_lwa_df_nosite, REML = T)
#summary(space_mod)

space_mod_site <- lmer(water_potential ~ lwa_g_cm2_centered  + (lwa_g_cm2_centered|week) + (1|tree) + (1|site), data = space_lwa_df_nosite,  REML = T)
...
space_mod <- lmer(water_potential ~ lwa_g_cm2 + (lwa_g_cm2|week) + (1|tree), data = space_lwa_df_nosite, REML = T)
#summary(space_mod)

space_mod_site <- lmer(water_potential ~ lwa_g_cm2  + (lwa_g_cm2|week) + (1|tree) + (1|site), data = space_lwa_df_nosite,  REML = T)
#summary(space_mod_site)
...
```{r, echo=FALSE}
space_mod <- lmer(water_potential ~ lwa_g_cm2_centered + (lwa_g_cm2_centered|week) + (1|tree), data =  wc_wk_centered_lwa_df, REML = T)
...
```{r, echo=FALSE}
space_mod_4 <- lmer(water_potential ~ lwa_g_cm2 + (lwa_g_cm2|week) + (1|tree) + (1|site), data =  wc_wk_centered_lwa_df, REML = T)
...
```{r, warning = F}
wc_wk_centered_pd_lwa_df <- wc_qudo_lwa_df %>% 
  filter(time == "pd") %>% 
  group_by(week) %>% 
...
space_mod_site <- lmer(water_potential ~ lwa_g_cm2_centered + (lwa_g_cm2_centered|week) + (1|tree), data = space_pd_lwa_df, REML = F)
#summary(space_mod)

space_mod <- lmer(water_potential ~ lwa_g_cm2_centered  + (lwa_g_cm2_centered|week) + (1|tree) + (1|site), data = space_pd_lwa_df,  REML = F)
#summary(space_mod_site)
...
space_mod <- lmer(water_potential ~ lwa_g_cm2 + (1|week) + (lwa_g_cm2|tree), data = space_lwa_df_nosite, REML = T)
#summary(space_mod)

space_mod_site <- lmer(water_potential ~ lwa_g_cm2  + (1|week) + (lwa_g_cm2|tree) + (1|site), data = space_lwa_df_nosite,  REML = T)
#summary(space_mod_site)
...
```{r}
wc_qudo_md_lwa_df <- wc_qudo_lwa_df %>% 
  filter(time == "pd")
  
time_mod <- lmer(water_potential ~ lwa_g_cm2 + (1|week) + (lwa_g_cm2|tree) + (1|site), data = wc_qudo_md_lwa_df)
#time_mod <- lmer(water_potential ~ lwa_g_cm2 + (lwa_g_cm2|tree), data = wc_qudo_md_lwa_df)
...
ran_lwa_df_tree %>% 
  ...
  labs(x = "Tree", 
       color = "Tree", 
       y = "Inetercept (Midday MPa ~ lwa_g_cm2)") +
theme(
  strip.background = element_blank(),
  strip.text.y = element_blank(), 
...
Just linear: 
```{r}
lin_wc_qudo_md_lwa_df <- wc_qudo_lwa_df %>% 
  filter(time == "pd") %>% 
  filter(time_season == "after leafout")
  
  
lin_time_mod <- lmer(water_potential ~ lwa_g_cm2 + (1|week) + (lwa_g_cm2|tree) + (1|site), data = lin_wc_qudo_md_lwa_df)
...
```{r}
bef_wc_qudo_md_lwa_df <- wc_qudo_lwa_df %>% 
  filter(time == "pd") %>% 
  filter(time_season == "before leafout") %>% 
  drop_na(site)
  
  
bef_time_mod <- lmer(water_potential ~ lwa_g_cm2 + (1|week) + (lwa_g_cm2|tree) + (1|site), data = bef_wc_qudo_md_lwa_df, REML = T)
...
```{r, echo=FALSE}
wc_qudo_pd_lwa_df <- wc_qudo_lwa_df %>% 
  filter(time == "pd")

time_mod <- lmer(water_potential ~ lwa_g_cm2 + (lwa_g_cm2|week) + (1|tree), data = wc_qudo_pd_lwa_df)
summary(time_mod)
```
