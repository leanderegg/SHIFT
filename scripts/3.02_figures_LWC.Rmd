...
space_slope_se <- ran_df_week_4 %>% 
 ...
 # filter(term == "lwc_mean") %>% 
  ...
  geom_point(aes(y = slope ,x = as.factor(week)), size =2) +
    geom_point(aes(y = upr, x = as.factor(week)), size = 1) +
  geom_point(aes(y = lwr, x = as.factor(week)), size = 1) +
  #geom_line(aes(group = as.factor(week), y = as.factor(week), x = upr)) +
  geom_segment(aes(y= lwr, 
                   yend = upr, 
                   x = as.factor(week), 
                   xend = as.factor(week), 
...
ggsave(here("figures", "pv_slope_plot"), pv_slope, device = "jpg", width = 1, height = 3, dpi = 300)
```
...
combined_slopes_wide <- cowplot::plot_grid(space_slope_se, time_summary, hyd_slopes, 
                                   nrow = 1, 
...
ggsave(here("figures", "combined_plots_all"), combined_plots_all, device = "jpg", width = 10, height = 9, dpi = 300)
```
