...
lwa_raw0 <- read_csv(here(paste0("Data_",datver_lwa), "shift_lwa_new.csv"), show_col_types = FALSE) %>% 
  clean_names() %>% 
  mutate(tree_id = case_when(tree_id %in% c(1475) ~ 1478, 
...
lwa_raw <- merge(trees_sites_df, lwa_raw1, by = c("tree"))
...
wc_lwa_df <- merge(wc_long_lwa_all2, lwa_new_df, 
                   by = c("tree", "time", "week", "date_wc", "species"), 
                   all = T) %>% 
...
write.csv(wc_lwa_df_toenter, here::here("processed-data", "lwa_toenter_20240422.csv"))
...
wp_wc_df2 <- merge(wp_wc_df1, lwa_new_df2) %>% 
...
```{r}
plotly::ggplotly(wp_wc_rwc_alas_df %>% 
  filter(week == 17, species == "blue oak") %>% 
  ggplot(aes(y = mpa_mean, 
             x = lwa_g_cm2_new, 
             color = time, 
             shape = as.factor(date_wp),
             label = tree)) +
  geom_point())
```
...
#   # )) %>% 
#   mutate(week_cwc_link = case_when(  #so that the weeks of overflights overlap with the week #s of sample collection
#     #date %in% c("2022-04-20") ~ 17, 
#     week %in% c(8, 9) ~ 9, 
#     week %in% c(14, 13) ~ 14, 
#     week %in% c(17) ~ 18, #shift week 18 collected on 4/27 and mpas collected on 4/25 and 4/27
#     week %in% c(20) ~ 21,
#    # date %in% c("2022-03-15" & time %in% c("md")) ~ 11, #predawns were collected on week 10 technically, so we want them to have that cwc
#     TRUE ~ as.numeric(week)
#   )) %>% 
...
# cwc_mpa_raw <- analysis_bothspp_df %>% 
#   # mutate(week_cwc_link = case_when( #so that the weeks of overflights overlap with the week #s of sample collection
#   #   week %in% c(8, 9) ~ 9, 
#   #   week %in% c(14, 13) ~ 14, 
#   #   #week %in% c(17) ~ 16, 
...
# 
# cwc_lwc_all_df <- merge(cwc_raw, cwc_lwc_raw, by = c("tree", "week"), all = T) %>% 
#   distinct() %>% 
...
mpa_raw_cwc <- analysis_bothspp_df %>% 
  ungroup() %>% 
