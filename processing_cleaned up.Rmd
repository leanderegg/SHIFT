...
Week 12: 2022-03-25; Switched to not doing leaves, only doing bulk; still have multiple reps per bulk, maybe to correspond with water potentials? 
...
wc_df <- read_csv(here("processed-data", paste0("wc_alldates_",datver,".csv")), show_col_types = FALSE) %>% #contains LWC data
  ...
  filter(!(week == 21 & date_wc %in% NA)) %>% 
  filter(!(week == 29 & date_wc %in% NA)) %>%
  filter(!(week == 33 & date_wc %in% NA)) 
```
...
#make sure bulk vs. individual leaves are correctly coded; this is important because bulk leaves will not be summarized over a single date. 

lwa_raw <- lwa_raw0 %>% 
  #filter(is.na(bulk_or_individual_leaf)) %>%
  group_by(dry_scan_area, tree_id) %>%
  mutate(bulk_or_individual_leaf_old = bulk_or_individual_leaf) %>% 
...
lwa_new_df1 <- lwa_raw %>% 
  ...
  mutate(tree = tree_id, 
         ...
         rep = leaf_number) %>% 
  # mutate(bulk_or_individual_leaf = case_when(
  #   bulk_or_individual_leaf %in% c(NA) ~ "bulk",
  #    bulk_or_individual_leaf %in% c("y0") ~ "leaf",
  #   TRUE ~ as.character(bulk_or_individual_leaf)
  # )) %>%
  select(tree, date_wc, time, week, rep, lwc_leaf, lwc_bulk, dry_scan_area, bulk_or_individual_leaf) %>% 
  # mutate(dry_scan_area = case_when(
  #   dry_scan_area > 201 ~ NA_real_, 
...
lwa_new_df <- bind_rows(lwa_leaves_df, lwa_bulk_df, lwa_wk_37) %>% 
  select(-lwc_leaf, 
         -lwc_bulk, -dry_scan_area, #-bulk_or_individual_leaf, 
         -area_cm2, 
         -rep) %>% 
  distinct() %>% 
...
wc_long_lwa_old <- wc_df %>% 
  select(tree, week, time, date_wc, lwc_bulk, lwc_leaf)


####Combine those together and see if there is a mismatch 
...
#####These are ones that we should be sceptical of: 
...
```{r}
#SOMETHING is off here, and I don't know what!! Something with how dry masses are combined; might be okay to ignore for now since the issue is likely with how things are lining up in 
...
wc_lwa_df <- merge(wc_long_lwa_all2, lwa_new_df, 
  ...
  filter(tree > 100, # we dont care about shrubs right now
         sum_dm_g > 0, #these cant be correct
...
lwa_df <- wc_df_lwa %>% 
  mutate(lwa_g_cm2_new = case_when(
    ...
    tree %in% c(2009) & week %in% c(12) ~ NA_real_, #this area is wrong, much too low - lost a leaf? 
  ...
  mutate(lma_g_cm2_new = case_when(
    lwc_wrong %in% c("off") ~ NA_real_,
    tree %in% c(2383, 2384) & week %in% c(37) ~ NA_real_,
    tree %in% c(2009) & week %in% c(12) ~ NA_real_, #this area is wrong!
    tree %in% c(2086) & week %in% c(14) & time %in% c('md') ~ NA_real_,
...
# wp_check <- wp_df %>% 
#   filter(week %in% c(10) & tree %in% c(2377))
# 
wp_wc_check <- wc_lwa_df %>%
  filter(week %in% c(33) & tree %in% c(2369, 2367, 2379, 2383, 2011))
```

Checked on these WP and LWC, and they seem correct (e.g. no good reason to think they're outliers)

filter(week %in% c(29) & tree %in% c(2370))
filter(week %in% c(21) & tree %in% c(2343))

CHECK THESE ENVELOPES: 
filter(week %in% c(15) & tree %in% c(2087, 2088, 2090, 2085, 2093))
...
wp_wc_lwa_df1 <- merge(wp_mean_df, lwa_df, 
                  by = c("tree","time", "week", "species", "site", "plot"), 
                  all = T) %>% 
  filter(species != "ARCA", 
         species != "LEU") %>% 
...
```{r}
#get rid of outliers identified above: (UPDATE: no longer any LWA > 0.05 after resumming the areas, yay!)
 wp_wc_lwa_df <- wp_wc_lwa_df1 #%>% 
  # mutate(lwa_g_cm2_new = case_when(
  #   lwa_g_cm2_new > 0.05 ~ NA_real_,
  #   TRUE ~ as.numeric(lwa_g_cm2_new)
  # )) 

plotly::ggplotly(wp_wc_lwa_df %>% 
  ggplot(aes(y = mean_mpa, 
             x = lwa_g_cm2_new, 
             color = species, 
             label = tree),
...
###okay, these will get brought it later down the line!
```

####WC + WP

Merge water potential dataset and water content datasets from above

```{r}
wp_wc_df1 <-  merge(wp_df, wc_df,
...
#when there are bigish differences, replace the combined bulk+leaf with just the bulk; otherwise take the mean of both. 
  ...
  select(-dm_leaf_g, -wm_leaf_g, -mpa) %>% 
  distinct() %>% 
...
##get only the columns we are interested in from the LWA_new df, so that we can combine them with the OG data: 
lwa_new_df <- wp_wc_lwa_df %>% 
  select(tree, time, week, lwa_g_cm2_new, lma_g_cm2_new) 

wp_wc_df2 <- merge(wp_wc_df1, lwa_new_df) %>% 
  select(-dm_bulk_g, -wm_bulk_g) %>% 
  distinct()

write.csv(wp_wc_df2, here::here("processed-data", "wp_wc_df.csv"))
```
...
```{r}
#get just the columns that we want from the LWA dataset that we made above in the "New LWA" step: 
wp_wc_lwa_df1 <- wp_wc_lwa_df %>% 
  select(tree, species, time, week, mean_mpa, lwa_g_cm2_new, lma_g_cm2_new)

wp_wc_rwc_alas_lwa_new_df <- merge(wp_wc_rwc_alas_df1, wp_wc_lwa_df1, all = T) %>% 
  # select(tree, species, time, week, type, date_wp, lwc_mean, mpa_mean, lma_g_cm2, lma_g_cm2_new, lwa_g_cm2, lwa_g_cm2_new) %>% 
  distinct()

#look at how new LWA and old LWA compare: 
wp_wc_rwc_alas_lwa_new_df %>% 
...
#Make weird looking new_lwa values NA, and rename the dataset since it's getting annoyingly long. 
all_data_new_lwa <- wp_wc_rwc_alas_lwa_new_df %>% 
  mutate(lwa_g_cm2_new = case_when(
    lwa_g_cm2_new > 0.05 ~ NA_real_, 
...
```{r}
wp_wc_rwc_alas_df <- merge(trees_sites_df, all_data_new_lwa, all = T) %>% 
...
```{r}
#get our 'analysis df'  data so that it aligns with the CWC dates: 
cwc_lwc_raw <- analysis_bothspp_df %>% 
...
#Combine CWC data with lwc data: 
...
cwc_lwc_all_df <- merge(cwc_raw, cwc_lwc_raw, by = c("tree", "week_cwc_link", "week"), all = T) %>% 
  distinct() %>% 
...
cwc_analysis_df <-  cwc_lwc_all_df %>% 
   select(tree, site, species, week, 
          #lat, 
          #lon, 
          time, date_shift, date_wp, cwc, lwc_mean, lma_g_cm2, alas_cm2_per_mm2, lwa_g_cm2, #leaf_count, 
          week_cwc_link, water_potential, time_season) %>%
   distinct() %>% 
