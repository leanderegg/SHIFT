...
lwa_raw <- read_csv(here(paste0("Data_",datver_lwa), "shift_lwa_new.csv"), show_col_types = FALSE) %>% 
  clean_names()
lwa_new_df1 <- lwa_raw %>% 
    clean_names() %>% 
  mutate(tree = tree_id, 
  ...
  select(tree, date_wc, time, week, lwc_leaf, lwc_bulk, dry_scan_area, bulk_or_individual_leaf) %>% 
...
lwa_new_df <- bind_rows(lwa_leaves_df, lwa_bulk_df) %>% 
  select(-lwc_leaf, -lwc_bulk, -dry_scan_area, -bulk_or_individual_leaf, -area_cm2) %>% 
  distinct()

#now this is to get the dry weight info in the right spot: 
wc_lwa_long <- wc_df %>% 
 ...
 # select(-lwc_bulk, -lwc_leaf) %>% 
  ...
  select(1:6, date_wc, sum_water_wt_g, lwc_bulk, lwc_leaf) %>% 
  distinct()
...
wc_long_lwa_all <- merge(wc_lma_long, wc_lwa_long) %>% 
  select(-lwc_leaf)

wc_df_lwa <- merge(wc_long_lwa_all, lwa_new_df, by = c("tree", "date_wc", "time", "week"), all = T) %>% 
...
lwa_new_df <- bind_rows(lwa_leaves_df, lwa_bulk_df) %>% 
  select(-lwc_leaf, -lwc_bulk, -dry_scan_area, #-bulk_or_individual_leaf, 
         -area_cm2) %>% 
  distinct()
...
wc_long_lwa_all1 <- merge(wc_lma_long, wc_lwa_long) %>% 
  mutate(lwc_new = sum_water_wt_g/sum_dm_g, 
         #lwc_old_mean = mean(lwc_bulk, lwc_leaf, na.rm = T), 
         mismatch = lwc_new - lwc_bulk, 
         mismatch_leaf = lwc_new - lwc_leaf) #%>% 
  #select(-lwc_leaf)

wc_long_lwa_all <- merge(wc_long_lwa_all1, wc_df, all.x = T) %>% 
  select(-plot, -site, - date_wc, -rep) %>% 
  drop_na(mismatch)

mismatched_lwc_lwa <- wc_long_lwa_all %>% 
  filter(mismatch > .5 | mismatch < -0.5)

wc_df_lwa <- merge(wc_long_lwa_all, lwa_new_df, by = c("tree", "time", "week"), all = T) %>% 
  # select(tree, time, rep, week, date_wc, dry_scan_area, dm_leaf_g, wm_leaf_g, dm_bulk_g, wm_bulk_g) %>% 
  filter(tree > 100, 
         sum_dm_g > 0,
         !(date_wc %in% c("2022-02-28"))) %>% 
...
####WC + WP, LWA new (NOT IN MAIN COMBINING)
```{r}
...
write.csv(wp_wc_df1, here::here("processed-data", "wp_wc_df.csv"))
#view(wp_wc_df)
```
...
After this merge, LMA and Al:As will be duplicated since they need to be in rows for both predawn and midday to calculate LWA
...
wp_wc_rwc_alas_lwa_new_df <- merge(wp_wc_rwc_alas_df1, wp_wc_lwa_df1) %>% 
