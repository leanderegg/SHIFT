...
head(wc_df)
# Group the data by tree, week, and time, and filter out groups with only one replicate
filtered_df <- wc_df %>%
...
lwa_new_df <- bind_rows(lwa_leaves_df, lwa_bulk_df) %>% 
  select(-lwc_leaf, 
...
wc_lma_long <- wc_df %>% 
  ...
  distinct() %>% 
  ungroup() %>% 
  group_by(tree, time, week) %>%  #sum of all the weights of leaves for a given date/time
...
wp_wc_lwa_df <- merge(wp_lwa_df, wc_df_lwa, 
...
```{r}
wp_wc_rwc_df_all <- merge(wp_wc_df1, rwc_df,
