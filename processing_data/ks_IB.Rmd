...
Read in water potential data: - We measured Ks on stems collected on "2022-04-25" "2022-03-27" "2022-04-12" and "2022-05-23"
...
Read in Ks leaf dry mass data and calculate SLA: 
...
ggally_plot <- ks_data_wp_sla %>% 
  select(ks_g_s_mpa_max, ks_g_s_mpa_nat, k_g_mm_s_m_pa_nat, k_g_mm_s_m_pa_max, kl_g_s_mm_m_pa_nat, kl_g_s_mm_m_pa_max, sla_mm_g, lma_g_mm, alas, mean_md, total_area, sapwood_area, tree) %>% 
  as.data.frame() %>% 
  filter(sapwood_area != 0) %>% 
  mutate(tree_id = tree)

dlookr::diagnose(ggally_plot)

GGally::ggpairs(ggally_plot, ggplot2::aes(aes(color = tree))) #also not working? Error in is.finite(x) : default method not implemented for type 'list'
```
...
ggplot(aes(y = ks_g_s_mpa_nat, x = mean_md, color = tree)) +
  geom_point() + 
