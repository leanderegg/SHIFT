...
Something may be off with those very wet 2347s...look back at notes? Could have been after the rain event, so maybe we should use the MPa from right before the rain occured as that is more indicative of the stress they were under? 
...
```{r}
ks_data_wp_sla %>% 
  ggplot(aes(y = prop_0yr, x = date, color = tree))  +
  geom_point() + 
  stat_summary(fun.y="mean", geom="line", aes(group=factor(tree))) +
